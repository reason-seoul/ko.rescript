...
这将你的 ReScript 编译成 JavaScript，然后使用 Node.js 运行得到的 JavaScript。**我们建议你使用我们独特的工作流程，在编辑器中为生成的 `.bs.bs` 文件保留一个标签**，这样你就可以学习 ReScript 如何转换为 JavaScript。能输出可供检查和学习的干净的 Javascript 代码的语言可不多！
...
在开发过程中，与其每次都运行 `npm run res:build` 来编译，不如使用 `npm run res:start` 启动一个观察器，在文件修改后自动重新编译。
