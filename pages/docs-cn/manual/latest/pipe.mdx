...
管道被用来模拟面向对象的编程。例如，`myStudent.getName`在 Java 等其他语言中是`myStudent->getName`在 ReScript 中(相当于`getName(myStudent)`). 这使我们能够拥有 OOP 的可读性，而没有拖入一个巨大的类系统只是为了在一个数据上调用一个函数。
