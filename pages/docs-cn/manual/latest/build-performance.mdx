...
ReScript 认为安装时，构建时和运行时的性能都很重要；性能属于那种平时你不在意，但是一旦缺失你却一下就能感受到的事情。
...
有的时候，由于一些混乱的基础设置，你的构建可能会很慢。我们通过 `bstracing` 为你提供一个交互式可视化的构建性能：
...
ReScript 自身在底层使用了一个名为 [Ninja](https://ninja-build.org) 的构建系统。Ninja 和 Make 类似，但是相比于其他成熟的构建系统来说，它是跨平台的、最小化的、专注于性能的，并且就是为更底层的构建模块而生的。在这方面，Ninja 是 `rescript` 的一个很棒的实现细节。
...
ReScript 读取 `bsconfig.json` 并在 `lib/bs` 中生成 Ninja 构建文件。该文件包含底层的的编译器命令、命名空间规则、中间产物的生成和其他内容。最后运行 `ninja` 进行实际构建。
...
`rescript` 本身负责一些杂七杂八的任务，还有观察器的 Node.js 封装。底层的、无观察者的，且快速的原生 `rescript` 被称为 `rescript.exe`。它位于 `node_modules/rescript/{your-platform}/rescript.exe`。
...
如果你不需要观察器，你可以运行上面的 `rescript.exe`。这就避免了 Node.js 漫长的启动时间，它的启动时间可能在 `100ms` 左右。我们的编辑器插件会找到并使用这个本地的 `rescript.exe`，以获得更好的性能。
...
小型项目构建的原始 `rescript.exe` 的构建时间应该在 `70ms` 左右。当你使用附带观察器的 JS `rescript` 的包装器时，这个速度会翻倍，因为你不需要在每次修改时手动运行构建（尽管你应该选择原始 `rescript.exe` 用于编程用途，例如将 rescript 插入到你现有的 JS 构建流水线）。
...
在一台 Retina Macbook Pro Early 2015（3.1 GHz Intel Core i7）上，我们使用 https://github.com/rescript-lang/build-benchmark ，在一个拥有 10,000 个文件的大型项目（2 个目录，每个 5000 个文件，前 5000 个没有依赖项，后 5000 个 10 个依赖前一个目录的文件）上，对 `rescript.exe` 进行了压力测试。
...
`rescript` 是基于文件的构建系统。尽管在内存中构建会极大的提升构建速度，但是我们选择不在内存中构建。在内存中构建存在内存泄漏、内存耗尽、构建半途中断以及其他风险。我们的观察者模式可以整天整月地开着，不会出现内存泄漏。

<!--
The watcher is also just a thin file watcher that calls `rescript.exe`. We don't like babysitting daemon processes.
-->

观察器实际上也是一个调用 `rescript.exe` 的小型文件观察器。我们不喜欢保姆级的守护进程。
...
简而言之，得益于我们的编译器和构建系统架构，我们能够**只构建需要构建的内容**。例如，如果 `MyFile.res` 没有改变，那么它就不会被重新编译。你可以在 JavaScript 等语言中粗略地模仿这种渐进方式，但不幸的是，其正确程度很低。例如，如果你重命名或移动一个 JS 文件，那么观察器可能会感到困惑，就无法找到“新”文件，或不能正确地清理内容，导致你需要清理你的构建并重新构建一次，这就没有达到增量的目的。
...
ReScript 使用接口文件的概念（`.resi`）（等同于[模块签名](module.md#signatures)），只公开你需要的东西，这自然会加快增量构建的速度。例如，如果你改变了一个 `.res` 文件，而该文件对应的 `.resi` 文件没有暴露所改变的部分，那么你就减少了你必须重构建的依赖文件的数量。
...
不幸的是，如今 JS 构建系统通常是构建一个 JS 项目的瓶颈。如果构建其他部分需要数秒钟或数分钟，那么即使构建的一部分的速度快得惊人也并没有什么效果。下面是几个建议：
...
- 谨慎引入更多的依赖项：库、语法转换（例如非官方支持的 PPX）、构建步骤加载器等。与提供的 API 效益相比，它们更可能会拖累编辑和构建体验
