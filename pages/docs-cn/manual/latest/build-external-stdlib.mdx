...
你的 ReScript 项目依赖于 `rescript` 包作为一个[`devDependency`](https://docs.npmjs.com/specifying-dependencies-and-devdependencies-in-a-package-json-file)，其中包括我们的编译器、构建系统和运行时，如`Belt`。然而，如果你发布你的代码，你不得不把它添加到 `package.json` 中的 `dependency`：
- 到 Docker 或其他低存储量的部署设备。
...
在这些情况下，`bs-platform` 的大小或仅仅是存在就会让人觉得麻烦，因为它不仅包括我们必要的运行时如`Belt'，还包括我们的编译器和构建系统。
...
为了解决这个问题，我们现在将我们的运行时作为一个独立的包发布在[`@rescript/std`](https://www.npmjs.com/package/@rescript/std)，其版本与 `bs-platform` 的版本一致。现在你可以把 `bs-platform` 作为一个 `devDependency` 而只把 `@rescript/std` 作为你的运行时 `dependency` 。
...
**这是一个高级功能**，请只在上述情况下使用它，如果你已经使用了具有消除死代码的 JS 打包器，你可能不需要这个功能。
...
打个比方，假设你想发布一个纯 JS 的 ReScript 9.0 库，可以像这样先安装所需要的包：
...
现在编译后的 JS 代码将引入 `external-stdlib` 定义的路径。检查 JS 的编译输出：
...
**请确保 `bs-platform` 和 `@rescript/std` 的版本号在你的 `package.json` 中匹配**，以避免由于不匹配的 stdlib 假设而遇到运行时问题。
