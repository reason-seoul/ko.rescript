...
BuckleScript是 ReScript 的旧品牌，更加注重对 JS 的适当支持和熟悉程度，而我们以前由于需要迎合各种不同的人群，所以无法达到我们想要的程度。

**What’s ReScript's relationship with OCaml?**

**ReScript和 OCaml 的关系是什么？**
...
参考[这里]（/blog/bucklescript-is-rebranding）。Reason是 OCaml 的一个语法层，BuckleScript也采用了这个语法层。目前的 ReScript 编译器也支持旧的 Reason 语法 v3.6，以便向后兼容。我们将长期支持它，以确保现有用户不会得到破坏性的改变。

**I come from Reason/OCaml. Will ReScript keep supporting X?**

**我来自Reason/OCaml，ReScript会继续支持 X 吗？**
...
现有的 Reason 语法是由不同的团队拥有的，有着不同的愿景。Reason的目标是与 OCaml 语法100%兼容，并支持所有版本的OCaml。在过去的几年中，我们得出的结论是很难在不牺牲用户体验的情况下实现这样的目标。另一个原因是，我们觉得作为一个团队，最好能有相同的愿景，这样我们就能做出更一致的决定。
