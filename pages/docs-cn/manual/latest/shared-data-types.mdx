...
- Option：`option` 类型的 `None` 值会编译为 JS 的 `undefined`。`Some` 值，比如 `Some(5)`，会编译为 `5`。同样地，您可以将传入的 JS `undefined` 作为 `None`。**ReScript 不处理 JS `null`**。如果你的 JS 值可能为 `null`，请使用 [Js.Nullable](api/js/nullable) 中的辅助函数
