...
- 锁定版本的依赖项（这些是你的本地包，它们应该在构建顶层包时总是被重新构建，它们应该列在 `bs-dependencies` 和 `pinned-dependencies` 里）
...
当一个包被构建时（`rescript build`），构建系统会构建顶层包和与之相关的锁定版本的依赖项。这样锁定版本的依赖中的任何修改都会反映到最终的 app 中。
...
记下这些知识，让我们用一些具体的例子来观察一下锁定版本的依赖的行为。
...
现在，当我们在 `app` 包中运行 `rescript build` 时，编译器总是会重新构建它的锁定版本的依赖项。
...
**重要：**ReScript 在监视模式中不会对任何的 `pinned-Dependencies` 进行重新构建！这是由文件监视的复杂性导致的，所以你需要建立自己的文件监视进程来在某些特定文件更改时运行 `rescript build`。
