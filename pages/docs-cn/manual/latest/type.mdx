...
ReScript 通过右侧的值 `10`，知道 `score` 变量是 `int` 类型。这被称作**inference**。同样，它也知道 `add` 函数接受两个 `int`值并返回了一个 `int` 值，判断的来源是在 `int` 上做加法的 `+` 运算符。
...
## Type Parameter (Aka Generic) | 类型参数（泛型）
...
如果没有类型参数，标准库将需要定义`arrayOfString`，`arrayOfInt`，`arrayOfTuplesOfInt`等类型，很没意思。
...
ReScript 的类型系统非常健壮，不允许出现危险的或不安全的东西，如隐式类型转换，随机猜测值的类型等。不过出于实用主义的考虑，我们为您提供了一个逃生通道，以便您向类型系统“撒谎”：
...
显然，**不要**过度使用这个特性。而是在处理现有的，过于动态的 JS 代码时，有品位地使用它：

More on externals [here](external.md).

关于 externals 的[更多细节](external.md)
