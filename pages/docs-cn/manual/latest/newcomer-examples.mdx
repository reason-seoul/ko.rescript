...
本节专门是为试图了解一般惯用写法和编码规范的新人准备的。如果你是一个初学者，对一个例子有很好的想法，请提出一个编辑建议！
...
对于一个其参数可能被传递 `null` 或 `undefined` 的 JavaScript 函数，一般将其转为 `option`。可以通过 ReScript 的 [`Js.Nullable`](api/js/nullable#t) 模块完成转换。在这种情况下，使用 `toOption`：
