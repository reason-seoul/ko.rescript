...
`rescript`只负责编译；它将 `.res` 或 `.resi` 文件映射为 JS 输出文件。因此，从理论上讲，不需要从我们这边整合构建系统。从 webpack 监听的角度来看，ReScript 生成的 JS 文件几乎等同于你手写的 JS 文件。我们还建议 **签入那些 ReScript 生成的 JS 文件**，因为这种工作流程意味着：

- 您可以在不影响现有的基础架构下，将 ReScript 静静地引入您的代码库中。
- 当你更新 `.res` 文件并更新 JS 构建制品时，你会对 JS 文件的性能和正确性感受到 **肉眼可见** 的不同。
- 紧急情况下，你可以让队友对 JS 文件进行热修复，而不需要先开始学习 ReScript。
- 您可以从您的代码库中完全删除 ReScript，所有事情将照旧（以防您的公司因任何原因决定停止使用我们）。
...
-->
加载器确实很麻烦；这种时候请在文件顶部使用如 `%raw("require('./myStyles.css')")` 这样的命令。这会使 [`raw`](embed-raw-javascript.md) 将代码段编译成一个实际的 JS require。
...
`rescript` 为每个 `MyFile` 源文件生成一个 `MyFile.d` 文件；你可以在 `lib/bs` 中找到它们。文件内容包括 `MyFile` 依赖列表，这个文件既可读也对机器也友好的。你可以按你的目的读取这些文件（但要注意 IO 的开销）。直接使用这些文件而不要自己创建依赖关系图；我们做了艰苦的工作，尽可能地跟踪这些依赖关系（包括内部模块、`open`s、模块名称重叠等等）。
