...
变体类型通常限制于一组固定的构造器。在极少数情况下，您可能仍然希望能够在初始化声明类型后给变体类型添加构造器。为此，我们提供了可扩展变体（Extensible Variant）类型。
...
**有趣的是：**在 ReScript 中，[异常](./exception) 实际上是可扩展变体实现的，因此 `exception UserError(string)` 与 `type exn += UserError(string)` 等价。这是极少数可扩展变体的有意义的用例之一。
...
我们通常建议尽可能坚持使用[普通变体](./variant)，以获得穷尽模式匹配的好处。
