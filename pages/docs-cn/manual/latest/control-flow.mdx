...
**`if-else`和三目运算符在 ReScript 中的使用比其他语言少得多**；[模式匹配](pattern-matching-destructuring.md)杀死了一大类以前需要条件表达式的代码。
...
在 ReScript 中没有跳出循环的 `break`关键字(也没有从函数中提前返回的`return`). 然而，我们可以通过使用[绑定可变值](mutation.md)轻松地脱离 while 循环。
