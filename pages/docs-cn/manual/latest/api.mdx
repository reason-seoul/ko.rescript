...
- [Js](api/js)：提供你所熟悉的 JavaScript API 绑定
- [Belt](api/belt)：提供额外的 JavaScript 中所没有的额外内容与辅助功能
- [Dom](api/dom)：提供 DOM 相关的类型和模块
...
- 对于那些在标准 JavaScript 中不存在的 API（当然也不存在于 `Js` 模块中），可使用 Belt。如果有的话尽量使用 Js，比如使用 `Js.Array2` 而不是 `Belt.Array`
- `Dom` 模块包含了用于用户空间 DOM 绑定的标准化类型。但由于 DOM 本身的复杂性，我们不建议你使用该模块，而是为您的应用创建自己的 DOM 绑定
...
**注意**：不推荐使用标准库的其他替代方案（除非是 DOM 绑定），因为这可能会导致混乱并分裂社区。
