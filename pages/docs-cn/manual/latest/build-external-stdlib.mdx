...
你的 ReScript 项目依赖于 `rescript` 包 [`devDependency`](https://docs.npmjs.com/specifying-dependencies-and-devdependencies-in-a-package-json-file)，其中包括我们的编译器、构建系统和运行时，如 `Belt`。然而，如果你准备以如下方式发布你的代码，你就不得不把它添加到 `package.json` 中的 `dependency` 中：
...
为了解决这个问题，我们现在将我们的运行时作为一个独立的包发布在 [`@rescript/std`](https://www.npmjs.com/package/@rescript/std)，其版本与 `bs-platform` 一致。现在你可以把 `bs-platform` 作为一个 `devDependency` 而只把 `@rescript/std` 作为你的运行时 `dependency`。
