...
> **注意：**在以前的版本 (< 8.3) 中，我们所有的互操作相关属性都以`bs.`前缀开始(`bs.module`, `bs.val`). 我们的格式化器将在较新的 ReScript 版本中自动删除它们。
...
你几乎可以把属性放在任何地方，你甚至可以通过像函数调用一样直观地使用它们来添加额外的数据，下面是几个著名的属性(在其他部分解释).
...
2. `@unboxed`注释了类型定义。
3. `@val`注释了`外部'语句。
4. `@as("aria-label")`注释了`ariaLabel`记录字段。
5. `@deprecated`注释了`customDouble`表达式。这在编译时显示一个警告，告诉用户不要长期依赖这个方法。
6. `@deprecated("Use SomeOther.customTriple instead")`注释了`customTriple`表达式，并用一个字符串来描述废弃的原因。
