...
本节专门是为试图弄清一般的惯用写法与编码规范的新人准备的。如果你是对示例有好点子的初学者，请提出修改建议！
...
如果将一个可能为 `null` 或 `undefined` 的 JavaScript 值传递给了函数，通常要将其转为 `option`。可以通过 ReScript 的 [`Js.Nullable`](api/js/nullable#t) 模块完成转换。在这个场景下使用 `toOption`：
