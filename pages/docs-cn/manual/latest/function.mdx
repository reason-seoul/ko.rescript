...
ReScript函数用箭头声明，并会返回一个表达式，就像 JS 中的函数一样。它们也会编译成简洁的的 JS 函数。
...
ReScript的函数是默认柯里化的，这是少数的编译到 JS 会产生的性能开销情况之一。编译器会尽最大可能地去除这些柯里化，然而，有一些边界情况，你可能想要保证柯里化不会发生。这些情况，可以放一个点`.`在函数的参数列表前：
...
**这个特性看起来是 trivial 的**, 但是这其实是我们作为一门函数式语言最重要的特性之一。如果你想要在 JS 的编译结果中移除所有运行时的`柯里化`, 我们鼓励你使用它。
