...
ReScript 自带一个快速且简洁的构建系统 [`rescript`](https://www.npmjs.com/package/rescript)，它被用作社区的权威构建系统。
...
**注意**：第三方库（指在 `node_modules` 中的库或 `pinned-dependencies` 引入的库）不会被监听，因为这样做可能超过 node.js 的监听数量限制。
...
**注意2**: 如果你想以 JS-monorepo-esque 的方式（`npm` 和 `yarn` 工作区）建立一个项目，你的子包的变化应该被构建工具注意到，你将需要在主项目的 `bsconfig.json` 中定义锁定版本的依赖。更多细节参见[这里](./build-pinned-dependencies)。
