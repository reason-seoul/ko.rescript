...
元组是JavaScript中不存在而ReScript独有的数据结构. 满足以下特征:
...
**注意**: 不存在大小为1的序对, 这种情况可以直接使用值本身来代替.

## Usage

To get a specific member of a tuple, destructure it:

为了获取元组的某个特定成员, 可以对元组进行解包:
...
尽量在**局部**使用元组. 对于长期存在并经常需要传递的数据结构最好使用**记录(record)**, 其有命名的成员字段.