...
本节专门为试图弄清一般的惯用写法与编码规范的新人准备。如果你是一个对示例有好的想法的初学者，请提出修改建议！

## Use the [`option` type](null-undefined-option.md) | 使用 [`option` 类型](null-undefined-option.md)
...
## Create a Parametrized Type | 创建参数化类型
...
## Modeling a JS Module with Default Export | 为带有默认导出的 JS 模块建模
...
## Checking for JS nullable types using the `option` type | 使用 `option` 检查 JS 的可空类型
