...
在 React 组件中有两种常见的副作用操作：需要清理的和不需要清理的。我们将在稍后的例子中研究它们的区别，但首先让我们看看 useEffect 接口。
...
`useEffect` 调用可能会接收额外的依赖项数组(参见 `React.useEffect1` / `React.useEffect2...7`)。只要依赖项之一发生变更，就会运行 effect 函数。关于这些接口的用处请看 [下面](#effect-依赖)。
...
在某些情况下，在每次渲染时进行清理或运行 effect 函数会产生性能问题。让我们看一个具体的例子，看看 `useEffect` 做了什么：
