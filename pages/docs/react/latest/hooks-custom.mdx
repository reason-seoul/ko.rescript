...
# Build A Custom Hook | 构建自定义Hooks
...
React自带了一些基础的hooks，如`React.useState`或者`React.useEffect`。这里你将学习如何针对React用例构建自己的高阶hooks。

</Intro>

## Why Custom Hooks? | 为什么自定义Hooks？

<!-- Custom hooks let you extract existing component logic into reusable, separate functions. -->
自定义hooks可以让你将现有组件的逻辑提取到可复用的，独立的函数中。
...
我们回顾一下之前的例子[React.useEffect section](./hooks-effect)，这个例子中我们构建了一个聊天应用的组件，用于展示消息，显示朋友是否在线。
...
现在假如我们的聊天应用有个联系人列表，我们期望用绿色展示在线用户的名字。我们可以复制粘贴上面相似的逻辑到`FriendListItem`组件中，但这并不明智。
...
以往在React中共用组件间状态逻辑我们有两种流行的方式：`render props`和高阶组件。现在我们看看Hooks是如果解决大多数同类问题的，同时也能避免你在组件树上增加更多的层级。

## Extracting a Custom Hook | 提取自定义Hook
...
在函数之间共享逻辑的话我们通常将逻辑提取到另一个函数中。组件和Hooks都是函数，所以这个思路也适用它们。
...
**自定义Hook是以”use”开头命名的函数同时也可能会调用其它的Hooks。**举例来说，下面的`useFriendStatus`是我们的第一个Hook（我们新创建了`FriendStatusHook.res`文件，同时用来存`state`类型）：
...
这里没啥新的内容-都是从上面组件中复制的逻辑。就像在组件中一样，确保只在自定义Hook的顶层无条件地调用其他Hook。
<!-- Unlike a React component, a custom Hook doesn’t need to have a specific signature. We can decide what it takes as arguments, and what, if anything, it should return. In other words, it’s just like a normal function. Its name should always start with use so that you can tell at a glance that the rules of Hooks apply to it. -->
与 React 组件不同，自定义 Hook 不需要具有特定签名。 我们可以决定它接受什么作为参数，以及它应该返回什么（如果有的话）。 换句话说，它就像一个普通的函数。 它的名称应该始终以 use 开头，以便你一眼就可以看出 Hooks 的规则适用于它。
...
我们的 `useFriendStatus` Hook的目的是为我们订阅朋友的状态。 这就是为什么它将 `friendId` 作为参数，并返回在线状态，如 `Online`、`Offline` 或 `Loading`：
```res
...
现在我们已经将这个逻辑提取到useFriendStatushook中，我们可以直接使用它：
...
**这个代码和之前的例子是等价的吗？**是的，它会以相同的方式工作。如果你仔细观察，你会注意到我们没有对行为进行任何更改。我们所做的只是将两个函数之间的一些公共代码提取到一个单独的函数中。自定义Hooks是自然遵循Hooks设计的约定，而不是React特性。
<!-- **Do I have to name my custom Hooks starting with “use”?** Please do. This convention is very important. Without it, we wouldn’t be able to automatically check for violations of rules of Hooks because we couldn’t tell if a certain function contains calls to Hooks inside of it. -->
**我必须以“use”命名我的自定义Hooks吗？**请这么做吧。这个约定非常重要。没有它，我们将无法自动检查是否违反Hooks规则，因为我们无法判断某个函数内部是否包含对Hooks的调用。
<!-- **Do two components using the same Hook share state?** No. Custom Hooks are a mechanism to reuse stateful logic (such as setting up a subscription and remembering the current value), but every time you use a custom Hook, all state and effects inside of it are fully isolated. -->
**使用同一个Hook的两个组件共享状态吗？**不。自定义Hook是一种复用状态逻辑的机制（例如设置订阅并记住当前值），但是每次使用自定义 Hook 时，其中的所有状态和效果都是完全隔离的。
...
**自定义Hook是如何获取独自的状态的？**对 Hook 的每次调用都会获得孤立状态。因为我们直接调用useFriendStatus，所以从React的角度来看，我们的组件只调用了useState和useEffect。而且正如我们之前所了解的，我们可以在一个组件中多次调用useState 和useEffect，它们将是完全独立的。

### Tip: Pass Information Between Hooks | Hooks之间传递信息

<!-- Since Hooks are functions, we can pass information between them. -->
由于Hooks是函数，我们可以直接在它们间传递信息。

<!-- To illustrate this, we’ll use another component from our hypothetical chat example. This is a chat message recipient picker that displays whether the currently selected friend is online: -->
为了说明这一点，我们将使用假设聊天示例中的另一个组件。 这是一个聊天消息收件人选择器，显示当前选择的朋友是否在线：
<CodeTab labels={["ReScript", "JS Output"]}>
...
这让我们知道当前选择的朋友是否在线。如果我们选择一个不同的朋友并更新`recipientId`状态变量，我们的`FriendStatus.useFriendStatus`Hook将取消订阅之前选择的朋友，并订阅新选择的朋友的状态。
...
自定义Hooks提供了共享逻辑的灵活性。你可以编写自定义Hook，涵盖广泛的用例，例如表单处理、动画、声明式订阅、计时器，以及可能更多我们没有考虑过的用例。更重要的是，你可以构建与React的内置功能一样易于使用的Hooks。
...
尽量避免过早地添加抽象。当涉及到足够多的状态逻辑处理时，组件会变得非常大是很常见的。这很正常——不要觉得你必须立即将其拆分为Hooks。但我们也鼓励你去发现自定义Hook可以将复杂逻辑隐藏在简单的接口后面或帮助解耦复杂组件的场景。
