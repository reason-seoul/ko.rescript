...
在Rescript中，我们为稍微加点标签，就可以轻易地绑定&调用 `draw` 函数： 
...
编译之后我们会得到和JS侧一样的函数，但有了标签语法的支持，在ReScript侧给函数传参时却清晰的多！
...
当需要将函数添加到JS的对象中（JS 模块除外），须要使用一种特殊的方式`send`：
...
是否在JS OOP中使用过 `foo().bar().baz()` 这种链式调用（“流式api”）？通过使用[管道操作符](pipe.md)，我们也可以在Rescript中这样用。
...
你或许有那种可以传递任意数量参数的JS函数。在这些不定数量参数性质相同（即具有相同的类型）的情况下，Rescript也支持传递可变参数。如果需要，添加 `variadic` 到你的 `external`。
...
如果您可以详尽地列举一个重载JS函数可采用形式，只需分别绑定：
...
如果你有不可抗拒的冲动说“如果这个JS函数参数是一个变量，而不是字符串或int类型”，那么好消息是：我们确实通过将参数注解为多态变量来提供这样的外部特性！假设您希望绑定以下JS函数：
...
显然，JS侧没有多态变量的概念！但在这里，我们只是依靠poly变量的类型检查和语法。实际上仅仅是针对类型的`@unwrap`注解，编译时会去掉变量的构造函数并仅仅留下payload中真实的值。请参考输出。
...
- 将`@string`附加到整个poly变量类型可以使其构造函数编译为同名字符串。
...
有时将预定好的的参数值传递给JS函数，使用`external`绑定到函数很方便：
...
`@as("exit")`和占位符`_`参数一起使用意味着你想让第一个参数编译为字符串`"exit"`。您还可以将任何JSON文本与`as`:`` @as(JSON`true`) ``、`` @as(JSON`{"name":"John"}`) ``等一起使用。
...
您还可以在JS输出中显式地“隐藏” `external` 声明的函数的某个参数，如果你想在不影响JS端的情况下向其他参数添加类型约束，这个特性就很实用：
...
（在像JS这样的动态语言中，curring操作是有风险的，因为在编译时如果忘了传递参数并不会报错）。
...
不幸的是，由于JS由于上述原因没有curring，ReScript多参数函数很难100%地清晰地映射到JS函数：

1. 当一个函数的所有参数都被传递（也称为无currying）时\*，ReScript会尽最大努力用最佳的方式进行编译，例如拥有3个参数的函数调用编译成一个传递3个变量的普通JS函数调用。

2. 如果很难检测函数应用是否执行完成，ReScript会使用运行时机制（“Curry”模块）来尽可能多地处理参数，并检查结果是否执行完成。

3. 一些JS API（如`throttle`、`debounce`和`promise`）可能会与上下文发生冲突，也就是使用函数`bind`机制、循环`this`等。这种实现方式回合curring逻辑发生冲突。
...
ReScript尝试尽可能多地执行步骤#1。即使当#1跳过并使用#2的curring机制时，通常也是没有负面影响的。
...
**然而**，如果你遇到#3，试探法不是最好的方法：需要一种有保障机制来完整地调用函数，而不需要中间的curring步骤。我们通过在函数声明和调用点上使用[“uncurring”语法](./function#uncurred function)来提供这种保障机制。
...
一般来说，推荐使用 `uncurry`；编译器在编译时将进行大量优化，以解决从curring到uncurring。然而，在某些情况下，编译器无法对其进行优化。在这些情况下，它将被转换为运行时检查。

## Modeling `this`-based Callbacks | 基于this的callback

Many JS libraries have callbacks which rely on this (the source), for example:

有很多JS库的回调函数依赖于this（源代码中），例如：
...
`this` 有它的第一个参数是`this`本身此保留的，对于参数为0的函数，声明时不需要冗余的unit类型。

## Function Nullable Return Value Wrapping | 函数可为Null类型的返回值
...
对于返回值可以是`undefined`或`null`的JS函数，我们提供`@return(...)`语法，可以自动将该值转换为`option`类型（请记住，ReScript`option`的`None`值仅编译为`undefined`而不是`null`）。
