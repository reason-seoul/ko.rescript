...
主要注解是 `@genType`，默认情况下是 _export_。

</Intro>

## Export and Import Types ｜ 导出和导入类型
...
从JS模块 `MyMath.ts` 中导入名为 `complexNumber` 的类型 (或`MyMath.js`)，使用 `@genType.import` 注解:
...
这个导入的类型将被ReScript.Ω视为不透明的
## Export and Import Values ｜ 导出和导入值
...
相似的，导入具有不同 JS 名称的值，使用例如 `@genType.import(("./MyMath", "ValueStartingWithUpperCaseLetter"))` 的方法
<!-- To import nested values, e.g. `Some.Nested.value`, use e.g. `@genType.import(("./MyMath", "Some.Nested.value"))`. -->
导入嵌套值，例如 `Some.Nested.value`, use e.g. `@genType.import(("./MyMath", "Some.Nested.value"))`
## Interface (.resi) and Implementation (.res) files ｜ 接口 (.resi) 和实现 (.res) 文件
...
如果 `Foo.resi` 和 `Foo.res` 都存在，注解会从 `Foo.resi` 中取。本地模块也是如此:如果存在，则模块类型优先。
...
可以通过在 `Foo.resi` 顶部添加注解 `@genType.ignoreInterface` 来覆盖该行为。用例:向 JS 暴露实现细节，而不是向 ReScript 暴露。
...
但是，如果你想在 JS 中隐藏 `name` 和 `surname` 是字符串的事实，你可以用 `@genType.opaque` 注解来实现：
...
## Renaming, @genType.as, and object mangling convention. ｜ 重命名,@genType.as。和对象 mangling 约定。
...
**注意:** 从ReScript 7.0.0 版本开始，`@genType.as` 在记录字段将不鼓励使用，
因为它会产生运行时转换成本。取而代之 `@bs.as` 将被支持，零成本消耗。
...
可以对函数使用 `@genType.as`，尽管这只是为了向后兼容而维护，并且不能在函数式组件上使用:
```res
@genType
...
## Dependent Projects / Libraries ｜ 依赖的项目/库
...
例如，如果依赖项是 `"bs-dependencies": ["somelibrary"]`，并且 `somelibrary` 包含 `Common.res`，它在库中查找 `foo` 的类型:
