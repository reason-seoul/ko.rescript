...
这段代码使用 `React.ref` 储存 `input` DOM 节点的引用，用于在按钮被点击时给 text field 设置焦点：
...
- 在 `focusInput` 函数中，我们首先需要验证 DOM 元素是否已经设置，然后使用 `focus` binding 来设置焦点。
...
React 将在组件 mount 时为 DOM 元素分配 `current` 字段，并在组件 unmount 时将其分配回 null。
...
我们建议使用[Ref转发](./forwarding-refs)。**Ref 转发允许组件选择将子组件的 Ref 暴露给自己**。你能在 Ref 转发的文档中找到将子组件的 DOM 节点暴露给父组件的例子。
...
不同于传递由 `React.useRef()` 创建的 ref 值，你会传递一个回调函数。这个函数接收 `Dom.element` 作为参数，以使它能在其他地方被存储和访问。

<!-- **Note:** Usually we'd use `React.Ref.domRef()` to pass a ref value, but for callback refs, we use `React.Ref.callbackDomRef()` instead. -->
**注意：**通常我们会使用 `React.Ref.domRef()` 传递一个引用值，但是对于回调 refs，我们使用 `React.Ref.callbackDomRef()`。
