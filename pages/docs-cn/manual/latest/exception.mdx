...
异常只是一种特殊的变体，在出现**异常**情况下抛出（不要滥用它们！）。
...
为了区分 JavaScript 异常和 ReScript 异常，ReScript 将 JS 异常命名为 `Js.Exn.Error(payload)` 变体。下面的代码演示了如何捕获一个从 JS 端抛出的异常：
...
`raise(MyException)` 引发一个 ReScript 异常。如果要引发一个 JavaScript 异常（不管你的目的是什么），可以使用 `Js.Exn.raiseError`：
...
前面的部分没有你想象的那么有用；为了让你的 JS 代码与抛出异常的 ReScript 代码一起工作，后者实际上不需要抛出一个 JS 异常。ReScript 的异常可以被 JS 使用！
...
上面的 `BadArgument` 异常接受一个内联记录类型。为了符合人体工程学（指用起来更符合直觉），我们将该异常特别编译为 `{RE_EXN_ID, myMessage, Error}`。如果异常接受普通的位置参数，比如标准库的 `Invalid_argument("Oops!")` 只使用一个参数，那么这个参数将被编译为 JS 的字段 `_1`，第二个位置参数将编译为`_2`，以此类推。
...
-->
### 在一个 `catch` 子句中同时捕获 ReScript 和 JS 异常
