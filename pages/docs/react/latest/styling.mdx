---
title: 样式
description: "ReScript & React 中的样式编写"
...
React 为内联样式提供了内置支持，但也有许多用于设计 React 组件样式的第三方库。你可能想要以下配置：
...
ReScript 也能兼容这些库。下面几节，我们分享了关于如何使用流行的库的一些想法。如果你想要使用我们没提及的库，可以搜索 [package index](https://rescript-lang.org/packages) 或在 [ReScript论坛](https://forum.rescript-lang.org) 发帖。
...
从90年代开始，内联样式就是编写样式的最基本形式。你能通过 `ReactDOM.Style.make` API 来为任何 DOM 元素提供一个 `style` 属性：
...
它是一个使用 [标签参数](/docs/manual/latest/function#labeled-arguments) 的函数调用（因此是类型化的），映射到熟悉的 CSS 样式对象 `{color: '#444444', fontSize: '68px'}`。对于 CSS 规范中的每个 CSS 属性，在 `make` 函数中都有一个驼峰命名的带标签的参数。
...
上面的 `Style.make` API 会安全地检查每个样式字段的类型！但我们可能忽略了一些极少使用的字段。如果是这样，类型系统将会提示你尝试添加的字段不存在。为了提供补救措施，我们提供了 `ReactDOM.Style.unsafeAddProp` 来不安全地给样式添加字段。
...
[CSS modules](https://github.com/css-modules/css-modules) 能像任何 JS 模块一样被导入。导入的是一个属性名和 CSS 文件的类名相同的 JS 对象。

<!-- As an example, let's say we have a CSS module like this: -->
例如，我们有个像这样的 CSS module：
...
**注意：** `{..}` 是一个开放的 [JS 对象类型](/docs/manual/latest/object#type-declaration)，所以类型检查器不会检查类名是否正确使用。如果你想要强制让编译器报错，将 `{..}` 替换为具体的 JS 对象类型，例如 `{"root": string}`。
...
与 [TailwindCSS](https://tailwindcss.com) 类似的 CSS 工具库通常需要一些全局导入的 CSS。
...
为了演示，让我们创建一些简单的绑定到 [`emotion`](https://emotion.sh/docs/introduction) (像 [这里](https://github.com/bloodyowl/rescript-react-starter-kit/blob/eca7055c59ba578b2d1994fc928d8f541a423e74/src/shared/Emotion.res) 提到的):
...
这会让你直接访问 `emotion` 的 API。下面是如何使用它们的例子：
...
请注意，这种方法不会检查 CSS 属性名是否有效。如果你想要确保只有有效的 CSS 属性才能被传递，你可以像这样定义你的 `css` 函数：
...
这里我们使用现有的 `React.Style.t` 类型来强制确保只传递有效的 CSS 属性名。

最后但同样重要的是，你也能绑定到可以让你直接使用原生 CSS 的函数：
...
请记住，API 的类型安全程度有一个范围（虽然处理起来更加 / 不那么复杂），所以选择一个满足团队需求的解决方案。
