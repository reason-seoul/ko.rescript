...
Rescript 拥有[标签参数](function.md#labeled-arguments)（它也是可选的）。标签参数在 `external` 中也可以使用！你可以使用它来_修复_函数不明确的用法。假设我们在做这个模型：
...
在 Rescript 这边，我们稍微加上标签，就可以轻易地绑定和调用 `draw` 函数：
...
我们编译得到相同的函数，但现在由于有了标签，在 ReScript 端的用法更加清晰了！
...
**注意**：在这种特殊情况下，你需要在 `border` 后面加上 `()`，因为 `border` 是[最后一个可选的参数](function.md#optional-labeled-arguments)。如果没有 `()` 去标识传参终止，那将会产生一个警告。
...
请注意，你可以自由地重新排列 ReScript 端的标签；它们总是正确地以声明的顺序出现在 JavaScript 输出中：
...
附加在 JS 对象上的函数（JS 模块除外）需要用一种特殊的方式进行绑定，使用 `send`：
...
你或许有接受任意数量参数的 JS 函数。ReScript 支持对这些函数进行建模，条件是任意参数部分是同质的（也就是类型相同）。如果需要，你可以添加 `variadic` 到你的 `external`。
...
`module` 将会在 [从 JS 导入/导出到 JS](import-from-export-to-js.md) 章节中进行解释。
...
除了上面的特殊情况，一般的 JS 函数在参数类型和数量上是可以任意重载的。你该怎样绑定这样的函数呢？
...
如果你可以穷举一个重载 JS 函数的可能形式，那就只需对每一种不同的形式进行绑定：
...
如果你对“要是这个 JS 函数的参数是变体，而不是 `string` 或 `int` 就好了”这个想法心痒难耐，那么好消息是：我们确实通过将参数标注为多态变体来提供这样的 `external` 特性！假设你希望绑定以下 JS 函数：
...
显然，JS 端不可能有多态变体参数！但在这里，我们只是借用了多态变体的类型检查和语法。类型的 `@unwrap` 标注会导致编译时去掉变体构造器，而仅留下载荷值。请看输出。
...
考虑一下 Node.js `fs.readFileSync` 函数的第二个参数，它可以接受一个字符串，但是只能从一个字符串集合中选取：`"ascii"`，`"utf8"`，等等。你当然可以字符串类型进行绑定，但是我们可以使用多态变体 + `string` 去进一步提高调用者的传参正确性：
...
当传递预先确定的参数值到 JS 函数时，使用 `external` 绑定一个函数有时是很方便的：
...
`@as("exit")` 和占位符 `_` 参数一起使用表示你想让第一个参数编译为字符串 `"exit"`。你也可以把任何 JSON 字面量与 `as` 一起用：`` @as(JSON`true`) ``，`` @as(JSON`{"name":"John"}`) ``，等等。
...
你还可以在 JS 输出中明确地“隐藏” `external` 函数参数，如果你想在不影响 JS 端的情况下向其他参数添加类型约束，这个特性就很实用：
...
看到 `addFive` 这个中间函数了吗？`add` 有 3 个参数，但只收到了 1 个。它被解释为柯里化了参数 `5`，并等待后面 2 个参数的被应用。函数类型签名如下：
...
1. 当函数的所有参数都被提供（也就是没有柯里化）时，ReScript 会尽力以用最佳的方式进行编译，例如将有 3 个参数的函数调用编译成 3 个参数的普通 JS 调用
...
ReScript 尝试尽可能多地执行 #1。即使它放弃 #1 并使用 #2 的柯里化机制时，这通常也是无害的。
...
**然而**，如果你遇到了 #3，启发式不是最好的方法：你需要一种有保障的方法来完整地调用函数，而不需要中间的柯里化步骤。我们通过在函数声明和调用处使用[“去柯里化”语法](./function#uncurred function)来提供这种保证。
...
上面的解决方案是安全的、有保证的、性能良好的，但有时看上去有些累赘。我们提供了一个替代解决方案，如果：
- 你正在使用 `external`
...
一般来说，推荐使用 `uncurry`；编译器会在编译时做很多优化来将柯里化函数去柯里化。然而，在某些情况下，编译器无法对其进行优化。在这些情况下，它会被转换为运行时的检查。
...
有很多 JS 库的回调函数依赖于 this（这是问题的来源），例如：
...
这里的 `this` 是指向 `x` 的（实际上，这取决于 `onload` 方法是如何被调用的，但是讨论这个我们就偏题了）。将 `x.onload` 的类型声明为 `(. unit) -> unit` 是不对的。相反，我们引入了一个特殊的属性，`this`，它允许我们用下面的方式为 `x` 作类型声明：
...
对于返回值可以是 `undefined` 或 `null` 的 JS 函数，我们提供了 `@return(...)` 来自动将该值转换为 `option` 类型（回忆一下，ReScript `option` 的 `None` 值只会编译为 `undefined` 而不是 `null`）。
...
目前支持 4 个指令：`null_to_opt`、`undefined_to_opt_`、`nullable` 和 `identity`。
