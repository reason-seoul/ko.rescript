...
- 接受一个参数为 `unit` 且返回 `unit` 的函数（它在 JS 端变成了一个不接受任何内容并且不返回任何内容的函数（指返回 `undefined`））
- 和一个指定调用上述函数之前延时的整数值
- 返回一个数字，即 timeout 的 ID。这个数字可能很大，所以我们将其建模为浮点数，而不是 32 位整数
...
**上面的情况并不理想**。想想怎么让 `setTimeout` 返回的 `float` 来作为 `clearTimeout` 的输入。我们没办法保证你传递给 `clearTimeout` 的参数是 `setTimeout` 的返回值！据我们所知，有人可能直接把 `Math.random()` 传递给后者。
...
显然，`timerId` 是一个只能通过 `setTimeout` 创建的类型！现在我们可以保证 `clearTimeout`_将_被传递一个有效的 ID 了。它是否是数字现在只是一个实现细节。
...
如果你想要绑定一个全局模块内的值，例如 `Math.random`，可以将 `scope` 附加到 `val` 属性的 `external`：
...
您可以通过传递一个元组给 `scope` 来实现绑定一个任意深度的对象：
...
像 `__filename` 和 `__DEV__` 这样的全局值并不总是存在；你甚至不能将它们建模为 `option`，因为仅仅是在 ReScript 中引用它们（然后编译为 JS）就会触发常见的 `Uncaught ReferenceError: __filename is not defined` 错误，例如浏览器环境就会这样。
