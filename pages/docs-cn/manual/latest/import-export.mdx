...
上面的代码中引入了 `Student.res` 中的 `message` 绑定。每个 ReScript 文件同时也是一个模块，所以获取其他文件的内容和获取其他模块的内容是一样的！
...
默认情况下，文件中的类型声明，绑定和模块都会导出，也就是可以由另一个文件直接使用。**这也就意味着这些值一旦被编译到 JS 就可以直接被 JS 代码使用**。
...
如果想要了解如何导入 JS 模块以及如何导出内容给JS使用，可以参考 JavaScript 互操作部分的 [Import from/Export to JS](import-from-export-to-js.md)。
