...
在[API文档](api/js/string)中查看我们熟悉的 `Js.String` API。 因为一个 ReScript 字符映射字符串到JavaScript字符串，你可以混用或比较两个标准库中的字符串。
...
- 在习惯了 ReScript 作为替代品之后，你很快就会发现，其他疯狂的模式是多么可怕。
...
**注意**: Char不支持 Unicode 或UTF-8，因此不推荐使用
...
ReScript的正则表达式会简洁的编译到 JavaScript 的对等物。
...
- `!`: 逻辑非。
- `<=`, `>=`，`<`, `>`
- `==`: 结构化相等，深入的比较数据结构：`(1, 2) == (1, 2)` 等于 `true`。 很方便，但用的时候需要注意
- `===`: 引用相等，浅层的比较数据结构。 `(1, 2) === (1, 2)` 等于 `false`。 `let myTuple = (1, 2); myTuple === myTuple` is `true`。
- `!=`: 结构化不相等。
- `!==`: 引用相等。
...
-->
**当绑定到 JavaScript 数字时要小心！** 由于 ReScript 整数的范围比JavaScript数字小得多，在处理大数字时，数据可能会丢失。 在这种情况下，将数字绑定为**float**要安全得多。 当绑定到 JavaScript 日期和它们的epoch time时要特别注意这一点。
