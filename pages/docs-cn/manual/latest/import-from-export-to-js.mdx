...
**注意**：由于 JS 生态的模块兼容性问题，我们建议将 ReScript 文件编译后的 JS 输出打开在一个选项卡中，**这比任何方法都好用**。因为你不想微妙地输出错误的 JS 模块导入/导出代码，除此之外还必须处理 Babel/Webpack/Jest/Node 的 CommonJS <-> ES6 兼容性的小问题。
...
如果您的 JS 项目使用 ES6模块，你可以导出和导入一些默认值：
...
JavaScript 默认导出实际上只是隐式地称为 `default` 的命名导出的语法糖（现在你知道了！）。因此，要从 ReScript 导出默认值，只需执行以下操作：
...
如果你的 JavaScript 的 ES6 默认导入是通过 Babel/Webpack/Jest 编译到 CommonJS 的 `require` 的，我们也为你处理好了这个问题！请参见 `__esModule` 的 CommonJS 输出选项卡。
