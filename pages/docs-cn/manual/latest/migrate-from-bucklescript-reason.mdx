...
- ReScript 几乎就是改了个名字的 BuckleScript，Rescript 提供的新语法很想像 Reason 的语法，但更加适合 Javascript 群体
- 即使你不升级，所有现有代码仍将正常工作
...
## Difference With Old Reason | 与 Reason 的不同之处
...
- 旧内插字符串：`{j|hello ${name}|j}` 变为 `` j`hello ${name}` ``。现在有了更好的unicode支持！
- 新内插字符串：`` `hello world` ``，同样支持多行和 unicode。`"hello world"` 现在仅支持单行字符串
...
- first class 模块：`(module S: Student)` 变为 `module(S: Student)`
