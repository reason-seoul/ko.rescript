...
ReScript本身使用了一个名为[Ninja]（https://ninja-build.org）的构建系统。Ninja就像 Make 一样，但它是跨平台的、最小的、专注于性能的的，并且注定是一个底层的构建模块，而不是一个完整的构建系统。在这方面，Ninja是`rescript'的一个伟大的实现细节。
...
一个小项目的原始`rescript.exe`构建时间应该在`70ms`左右。当你使用 JSf 封装 `rescript`时，这个速度会翻倍，因为你不需要在每次修改时手动运行构建（尽管你应该选择原始`rescript.exe`用于可编程用途，例如将 rescript 插入到你现有的JS构建管道）。
...
- 10k文件的无操作构建：`800ms`（检查10k文件的 mtimes 所需的最少时间）。
- 清洁构建：<3分钟。
...
简而言之，由于我们的编译器和构建系统的架构，我们能够**只构建需要的东西**。例如，如果`MyFile.res`没有改变，那么它就不会被重新编译。你可以在 JavaScript 等语言中粗略地模仿这种增量主义，但不幸的是，其正确程度很低。例如，如果你重命名或移动一个 JS 文件，那么观察者可能会感到困惑，从而不接受“新”文件，或不能正确地清理东西，导致你需要清理你的构建并重新构建一次，这违背了目的。
...
不幸的是，如今 JS 构建系统通常是构建一个JS项目的瓶颈。如果构建的其他部分需要几秒钟或几分钟，那么构建的部分速度快得惊人就并不那么重要了，这里有几个建议。
...
- 将更多的文件转换成ReScript =)。更少的文件通过更少的 JS 管道部分，这有很大的帮助。
- 小心引入更多的依赖性：库、语法转换（例如非官方支持的PPX）、构建步骤加载器等。这些拖累编辑和构建体验的东西可能会超过它们提供的 API 好处。
...
然而，热重载在本质上是脆弱的，并且偶尔不满足一致性（坏状态、坏求值等）和繁重的开发服务器设置/配置比它提供的好处更少。我们从谨慎和稳定的角度出发，决定 _暂时_ 不提供一个内置的热重载。**注意**：你仍然可以使用你的 JS 构建管道所提供的热重载设施。