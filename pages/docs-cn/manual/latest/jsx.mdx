...
这是传递元素为 `child1` 和 `child2` 的列表到子位置的语法。它会被去糖化（desugar）为一个包含 `child1` 和 `child2` 的列表：
...
再次**注意**，这不是 ReasonReact 做的转换；ReasonReact 将最终的列表变成一个数组，但是这样在思路上没问题。
...
所以很自然地，`<MyComponent> myChild </MyComponent>` 被去糖为 `@JSX MyComponent.createElement(~children=list{myChild}, ())`。也就是说，无论你做什么，传递到子位置的参数都会被包裹在一个列表中。如果你不希望这样呢？**如果你想直接传递 `myChild`，而不进行额外的包装**呢？
...
这将传递 `myChild` 的值，而不把它包装到一个列表中（在 ReasonReact 的情况下是数组），也就是去糖为：
...
这在你处理的 `myChild` 已经是一个列表的情况下是非常有用的，你可以直接转发而不需要额外包装它（这将是一个类型错误）\*。它还允许你在 `children` 位置传递任意的数据结构（记住，JSX 的 `children` 实际上只是一个常规的 prop）。
...
- 属性和子节点并没有强制要求 `{}`，但为了便于学习，我们还是展示了它们。一旦你格式化了你的文件，它们中的一些就会消失，一些变成了括号
- 没有对 JSX prop 平展的支持，比如 `<Comp {...props}}. />`。但我们有子节点平展：`<Comp> ...children </Comp>`
- 双关！（punning）
...
“双关”是指标签和值相同时的语法简写。例如，在 JavaScript 中，不用写 `return {name: name}`，可以用 `return {name}`。
...
Reason JSX 支持双关，`<input checked />` 只是 `<input checked=checked />` 的简写。格式化工具将帮助你尽可能地按照双关语的语法进行格式化，这在有很多 prop 需要传递的情况下很方便：
...
这样，每个人都能从 JSX 语法中受益，而不需要选择使用它的特定库，例如 ReasonReact。
...
\* 你可能会想，为什么在 ReactJS 中从来不需要这样的子节点平展呢；ReactJS 使用了一些特殊的运行时逻辑 + 特殊的语法转换 + 变量参数检测和标记，来避免大多数的这些情况（[看这里](https://github.com/facebook/react/blob/9b36df86c6ccecb73ca44899386e6a72a83ad445/packages/react/src/ReactElement.js#L207)）。这样的动态使用使类型系统的检查变得相当复杂。由于我们控制了整个语法和 ReasonReact，我们决定引入子节点平展来区分包装和不包装的情况，这样就没有编译时和运行时开销了！
