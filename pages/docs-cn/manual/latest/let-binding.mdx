...
ReScript的 `if`、`while` 和函数都使用相同的块作用域机制。下面的代码能够正常工作并不是因为有什么特殊的 “if 作用域”；而是因为它使用了上面提到的作用域语法和特性。
...
访问的绑定是向上查找的最近结果。这里没有值的修改! 如果你需要_真正的_修改，（比如传递一个值作为参数，让某些代码修改这个值），我们提供了一个稍显承重的[mutation特性](mutation.md)。
...
`%%private`也可用于到文件级别的模块，这样为了隐藏某些特定的值时，用户可以不用提供一个单独的模块接口文件。
