...
这是一个带有示例的专业词汇表。所有的功能都会在接下来的页面中进行描述。
...
> **注意：**在 ReScript 8.3 版本之前，全部属性（attributes）都是以 `bs.` 作为前缀的。在新的版本中不再需要使用该前缀，格式化工具会自动删除它。
...
- `@as`： [这里](attribute#usage)，[这里](bind-to-js-function#fixed-arguments)，[这里](bind-to-js-function#constrain-arguments-better) 和 [这里](generate-converters-accessors#usage-3)
...
- `@send`: [here](bind-to-js-function#object-method) 和 [here](pipe#js-method-chaining)
...
- [`@JSX`](jsx)
- `@react.component`：[这里](/docs/react/latest/introduction) 和 [这里](https://github.com/reasonml/reason-react)
...
-->
## 全局模块值（Global Module's Value）绑定
...
我们可以放弃 `option` 类型，转而使用 `Js.Nullable.t` 来处理那些可能是 `undefined` 和 `null` 的值：
