...
当你创建新的记录值时，ReScript 试图找到符合该值形状的记录类型声明。因此，这里的 `me` 值被推断为 `person` 类型。
...
**注意**: 平展不能向记录值添加新字段，记录的形状是由其类型固定的。
...
记录的字段可以是可变的，这允许你用 `=` 操作符高效地就地更新这些字段。
...
很多 JavaScript API 往往有庞大的配置的对象，如果用记录来建模这些对象可能有点令人讨厌，因为你总是需要在创建记录时指定所有记录字段。
...
1. 事实是，在你的应用程序中，大多数时候你的数据的形状实际上是固定的，如果不是，它有可能更好地表示为变体(接下来介绍) + 记录的组合
2. 由于记录类型是通过找到那个单一的显式类型声明来解决的（我们称之为“名义类型(nominal typing)”），类型错误信息最终会比对等的类型（“结构式类型(structural typing)”，如元组）更好。这使得重构更容易；改变一个记录类型的字段自然地允许编译器知道它仍然是同一个记录，只是在某些地方被误用。否则，在结构化类型下，可能会变得很难分辨是定义的地方还是使用的地方出了问题
