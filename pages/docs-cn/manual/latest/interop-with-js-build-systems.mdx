...
`rescript`只负责编译；它将 `.res` 或 `.resi` 文件映射为 JS 输出文件。因此，从理论上讲，不需要从我们这边整合构建系统。从 webpack 观察者的角度来看，ReScript 生成的 JS 文件几乎等同于你手写的 JS 文件。我们还建议 **首先检查那些 ReScript 生成的 JS 文件**，因为这种工作流程意味着：

- 你可以在不影响现有的基础架构下，将 ReScript 静静地引入您的代码库中
...
你可以在 `bsconfig.json` 中配置 `in-source` 和 `suffix` 字段使 ReScript JS 看上去更地道：
...
`rescript` 为每个 `MyFile` 源文件生成一个 `MyFile.d` 文件；你可以在 `lib/bs` 中找到它们。文件内容是既对人可读也对机器也友好的 `MyFile` 依赖列表。你可以按你的目的读取这些文件（但要注意 IO 的开销）。请使用这些文件而不要自己创建依赖关系图；我们做了艰苦的工作来尽可能地跟踪这些依赖关系（包括内部模块、`open`s、模块名称重叠等等）。
