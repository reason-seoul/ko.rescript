...
ReScript **最棒**的特性之一就是模式匹配了。模式匹配将 3 个极佳的特性整合到了一起：
...
甚至 JavaScript 也有析构功能，也就是"打开"一个数据结构，提取我们想要的部分，然后将它赋值到一个变量：
...
但是我们**非常不推荐**析构数组（这种情况最好用元组代替）。上面的例子会报错，它们在这里只是为了做补充说明。你即将在后面看到，析构数组和列表的正确方法是使用 `switch`。
...
虽然析构的模式匹配很好，但它并没有真正改变你理解代码结构的方式。使用基于数据形状的条件执行可以让你对代码的理解发生范式转换。
...
我们想要处理三种不同的情况。比如说，如果值是 `GoodResult(...)`，那就打印一条成功消息，如果值是 `NoResult` 那就做一些其他事情，等等。
...
在其他语言中，你可能会用一系列的 if-else 来实现上述功能，这样做成可读性不好而且很容易出错。在 ReScrit 中你可以使用强大的 `switch` 模式匹配来对值进行析构，然后基于析构的结果调用需要的代码块：
...
转眼之间，混乱的 if-else 判断转变为了线性代码列表，它非常干净而且会被编译器检验，它会根据值的形状执行相关的代码片段。
...
下面是一段真实场景的代码，在别的语言中表达相同的功能是很头疼的。比如对于这个数据结构：
...
在前面的 `person` 的例子中展示的嵌套模式检查也可以 `switch` 的顶层：
...
如果你有个像 `Teacher(payload)` 的值，你可能只想匹配 `Teacher` 部分而完全忽略 `payload` 部分，这种情况你可以使用下划线 `_`，就像这样：
...
这稍微有些啰嗦，但只是一次性的编写工作。当你为 `status` 类型添加了一个新的变体（比如 `Quarantined`）并需要更新模式匹配的地方时，这样做将有所帮助。顶层通配符会默默地继续工作，可能会产生意外的 bug。
...
有时你可能想对值做更多的检查，在值上进行一些任意的检查。你可能会尝试这样写：
...
**注意：**ReScript 9.0 版本之前使用的是 `when` 子句而不是 `if` 子句。ReScript 在 9.0 版本从`when`切换到了 `if`。（`when` 仍然可以使用，只是被废弃了）
...
如果函数可能会抛出一个异常（稍后会介绍），除了可以匹配正常的返回值之外你还可以对_异常_进行模式匹配。
...
**注意**: 你只能传递字面值（即具体的值）作为模式，而不能传递绑定的名字或其他东西，下面的做法不能像预期的那样工作：
...
如果上述特性还不够，ReScript 还提供了可以说是最重要的模式匹配特性：**编译期缺失模式检查**。
...
你看到我们移除的部分了吗？这一次，我们省略了对当 `name` 不是 Mary 或 Joe 时对 `person1` 是 `Teacher({name})` 的情况的处理。
...
未能对值的每一种情况都进行处理是大多数的 bug 的来源，当你重构别人编写的代码时这种事经常发生。幸运的是在 ReScript 中遇到这种情况时编译器会告诉你：
...
**BAM**！在你运行代码之前，你就已经抹去了一整类的 bug。事实上，大部分的空值都是这样处理的：
...
如果你不处理 `None` 的情况，编译器会发出警告。你的代码中不会再有 `undefined` 之类的错误了！
...
希望你能了解到模式匹配是如何通过简洁的析构语法、`switch` 中适当的条件处理和静态穷举检查来改变编写正确代码的游戏规则的。
...
避免不必要地使用通配符 `_`，使用 `_` 将绕过编译器的穷举检查。因此，当你在变体中增加一个新的情况时，编译器将无法通知你可能出现的错误。只在需要应对无限的可能性的时候使用 `_`，例如 string，int，等等。
...
现在要想在这段代码中犯错是非常困难的！当你想需要一个有许多分支的 if-else 时，最好用模式匹配来代替。它更简洁而且[性能也更好](variant#design-decisions)。
