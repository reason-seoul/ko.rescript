...
**模块就像迷你文件一样！**可以包含类型定义、`let` 绑定、嵌套模块等等。
...
使用 `module` 关键字来创建模块。模块的名字必须是**大写字母**开头。任何你可以放在 `.res` 文件的东西都可以放置在模块定义的 `{}` 块中。
...
模块中的内容（包括类型！）可以像记录一样用点 `.` 来访问。下面的代码演示了模块的名字空间功能。
...
总是通过模块来引用值/类型是很繁琐的。我们可以 `open` 一个模块并直接使用它的内容，而不必总是在内容前面加上模块的名称。除了这样做：
...
`School` 模块的内容在作用域内可见（**不是**复制到文件中，仅仅是可见而已！）。这样就可以正确的找到 `profession`、`getProfession` 和 `person1`。
...
有些情况下，由于重定义了现有的标识符（绑定、类型），`open` 将会导致警告。使用 `open!` 明确告诉编译器这是期望的行为。
...
**注意：**和 `open` 一样，如果没有必要就不要过度使用 `open!` 语句。通过（子）模块来解决遮蔽问题。
...
作为 `open` 模块的替代方法，你也可以将模块的函数和值析构为单独的 let 绑定（类似于我们在 JavaScript 中解构对象）。
...
**注意：**你不能使用模块析构提取类型 —— 你应该使用类型别名（`type user = User.myUserType`）。
...
在模块中使用 `include` 可以静态的“扩展”模块内容，因此 `include` 经常用来扮演“继承(inheritance)”和“混合(mixin)”的角色。

<!--
**Note**: this is equivalent to a compiler-level copy paste. **We heavily discourage `include`**. Use it as last resort!
-->

**注意：** 这相当于编译级别的复制粘贴。**我们强烈反对使用 `include`。**最好把它作为最后不得不用的手段！
...
**注意：** `open` 和 `include` 非常不同！前者将模块的内容带入到当中作用域中，这样你就不必每次都用模块名称作为前缀来引用值。后者静态的**复制**模块的定义，然后也会执行 `open` 操作。
...
**注意：**按照惯例 ReScript 文件名应该首字母大写，以便它们的名字写与模块名称匹配。未大写的文件名不是无效的，它们会隐式地转换为大写的模块名字。例如 `file.res` 将会被编译为 `File` 模块。为了简化和减少这种间断感，最好使用将文件名首字母大写的惯例。
...
模块的类型被称为“签名”，可以显式地编写。如果将模块比作 `.res`（实现）文件，那么模块的签名就类似于 `.resi` （接口）文件。
...
使用 `module type` 关键字来创建签名。签名必须**首字母大写**。任何你可以放在 `.resi` 文件的东西都可以放置在签名定义的 `{}` 块中。
...
签名定义了模块与签名匹配而必须满足的要求列表。这些要求的形式如下：
...
- `type t = someType` 表示需要等于 `someType` 的类型字段 `t`
- `type t` 表示需要一个类型字段 `t`，但是没有实际具体的类型要求。我们会在签名的其他条目中使用 `t` 来描述关系。例如 `let makePair: t => (t, t)`。但是我们不能假设 `t` 是 `int`。这给了我们强大的抽象能力
...
为了说明各种各样的类型项，请考虑上面的 `EstablishmentType` 签名，它要求一个模块满足：
...
`EstablishmentType` 类型的模块可以包含比此签名更多的字段，就像之前章节中的模块 `School`一样（如果我们选择为 `School` 赋予 `EstablishmentType` 类型。否则 `School` 会暴露所有字段）。这使得 `person1` 字段成为了实现细节！外部无法访问它，因为它不存在于签名中；签名**限制了**其他人对模块的访问。
...
隐藏底层类型作为其他人无法依赖的实现细节也很有用。如果你询问 `Company.profession` 的类型是什么，`Company` 不会公开变体，而只会告诉你它是 `Company.profession`。
...
像模块一样，模块签名也可以通过对其他模块签名使用 `include` 进行扩展。再次，**我们非常不建议这样做**：
...
**注意：** `BaseComponent` 是模块**类型**而不是实际的模块本身！
...
如果没有已定义的模块类型，则可以使用 `include (module type of ActualModuleName)` 提取模块的类型。例如，我们可以扩展标准库的 `List` 模块，它没有定义模块类型。
...
与 `React.res` 文件隐式地定义 `React` 模块类似，`React.resi` 隐式地定义 `React` 签名。如果没有提供 `React.resi`，`React.res` 的签名默认显示模块的所有字段。因为不包含实现文件，`.resi` 文件也用于记录对应模块的公共 API。
...
模块可以作为参数传递给函数！它相当于将文件作为一等公民传递。但是，模块与其他常见概念在语言的“层”上是不同的，我们不能将它们传递给常规函数。我们将它们传递给名为“仿函数”的特殊函数。
...
仿函数可以使用函数应用的语法。在本例中，我们创建了一个集合，他的子项为整数对。
...
与模块类型一样，仿函数类型也起到约束和隐藏仿函数的作用。仿函数类型的语法与函数类型的语法一致，但是使用首字母大写的类型表示模块的签名，它将作为仿函数的参数。在前面的实例中我们公开了集合的内部类型；通过给 `MakeSet` 一个仿函数签名，我们可以隐藏底层的数据结构！
...
请注意，其他 ReScript 模块不能访问具有奇异文件名的模块。
...
模块和仿函数相比于语言的其他部分（函数、let 绑定、数据结构等）处于不同的“层”。例如，你不能轻易的将它们传递到元组或记录中。如果需要的话请谨慎地使用它们！很多时候只需要记录或函数就够了。
