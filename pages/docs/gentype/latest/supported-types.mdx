...
当最多一个变体的情况下有一个有效载荷，如果有效载荷是对象类型，例如。
...
多态变体的处理方式类似。注意，多态变量的有效负载总是一元的: ``` `Pair(int,int) ``` 只有一个 `(int,int)` 类型的有效载荷。相反，普通变体区分一元``` Pair((int,int)) ``` 和二进制 ``` Pair(int,int) ``` 有效载荷。所有这些情况在 JS 中都表示为 ```{tag: "Pair", value: [3, 4]}```，转换函数负责不同的 ReScript 表示。
...
[ImmutableArray.res/.resi](https://github.com/reason-association/genType/tree/master/examples/typescript-react-example/src/ImmutableArray.resi), 目前需要添加哪些到您的项目中
...
如果 ReScript 类型包含类型变量，则不转换对应的值。换句话说，转换就是恒等函数。例如，类型为 `{payload: 'a} => 'a` 的 ReScript 函数，必须将有效载荷的值视为黑箱，作为参数多态的结果。如果使用类型化后端，则将 ReScript 类型转换为相应的泛型类型。
