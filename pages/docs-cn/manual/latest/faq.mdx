...
我们的目标是为JavaScript平台提供最好的类型语言体验。

**What’s the relationship with BuckleScript?**

**ReScript与BuckleScript有什么关系？**
...
BuckleScript是ReScript的旧品牌，更加注重对JS的适当支持和熟悉程度，而我们以前由于需要迎合各种不同的人群，所以无法达到我们想要的程度。

**What’s ReScript's relationship with OCaml?**

**ReScript和OCaml的关系是什么？**
...
我们重用并调整了优秀的类型系统和许多其他来自OCaml的高质量组件，以提高JS体验。
此外，ReScript还提供了自己的语法、构建系统、IDE、后端、JS互操作、额外的语言功能等。
...
参考[这里]（/blog/bucklescript-is-rebranding）。Reason是OCaml的一个语法层，BuckleScript也采用了这个语法层。目前的 ReScript 编译器也支持旧的 Reason 语法 v3.6，以便向后兼容。我们将长期支持它，以确保现有用户不会得到破坏性的改变。

**I come from Reason/OCaml. Will ReScript keep supporting X?**

**我来自Reason/OCaml，ReScript会继续支持X吗？**

Please see our [blog post](/blog/a-note-on-bucklescripts-future-commitments) on this matter.

请参考我们的[博文](/blog/a-note-on-bucklescripts-future-commitments)以了解此事。

**Where can I see the docs in old Reason/OCaml syntax?**

哪里可以看到旧版Reason/OCaml语法的文档？
...
请看我们在[Async & Promise](promise)页面中介绍的回答。
...
现有的Reason语法是由不同的团队拥有的，有着不同的愿景。Reason的目标是与OCaml语法100%兼容，并支持所有版本的OCaml。在过去的几年中，我们得出的结论是很难在不牺牲用户体验的情况下实现这样的目标。另一个原因是，我们觉得作为一个团队，最好能有相同的愿景，这样我们就能做出更一致的决定。
...
ReScript团队（张宏波，Cheng，Cristiano，Maxim，Patrick，Ricky）。

**We have a new forum; will we also have our own Discord?**

我们有一个新论坛；我们也会有自己的Discord吗？
...
暂时没有，我们发现太多重要信息会在 Discord 中随意传递，然后在噪音中消失。我们更喜欢人们在 [论坛](https://forum.rescript-lang.org) 上交流，这对不太活跃的成员更友好。

The team doesn't use the old Discord anymore. We encourage you to move your questions to the forum instead.

该团队不再使用旧的 Discord，我们鼓励您将问题移至论坛。
