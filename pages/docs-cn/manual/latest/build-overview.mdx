...
每次构建都会从你的项目的源文件中创建构建工件（artifacts）。

<!--
**To build a project (including its dependencies / pinned-dependencies)**, run:
-->

使用以下命令来**构建一个项目（包括其依赖/锁定版本依赖 (pinned-dependencies)）**：
...
**注意**：第三方库（指在 `node_modules` 中的库或 `pinned-dependencies` 引入的库）不被观察，因为这样做可能超过 node.js 的观察数量限制。
...
**注意2**: 如果你想以 JS-monorepo-esque 的方式（`npm` 和 `yarn` 工作区）建立一个项目，你的子包的变化应该被构建工具注意到，你将需要在主项目的 `bsconfig.json` 中定义锁定版本依赖。更多细节参见[这里](./build-pinned-dependencies)。
...
这将清空你的项目的构建工件。要清理依赖项的构建工件，可以使用：
