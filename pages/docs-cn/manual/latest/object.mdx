...
Although ReScript records compile to clean JavaScript objects, ReScript objects are a better candidate for emulating/binding to JS objects, as you'll see.
 -->
虽然 ReScript 的记录可以编译成简洁的JavaScript对象，但 ReScript 对象是模拟/绑定到 JS 对象的更好的候选，你马上就会看到了。
...
**Optional**, unlike for records. The type of an object is inferred from the value, so you never really need to write down its type definition. Nevertheless, here's its type declaration syntax:
 -->
不像记录，object的类型声明是**可选的**。一个对象的类型是由值推断出来的，所以你从来不需要写下它的类型定义。 尽管如此，这里是它的类型声明语法： 
...
-->
更新是不被允许的，除非该对象是一个来自于 JavaScript 端的绑定。在那种情况下，可以使用`=`更新对象：
...
-->
由于对象不需要类型声明，而且 ReScript 为你推断出所有的类型，所以你可以非常快速和容易地(同时也是危险地)绑定到任何JavaScript API。 看看 JS 的编译结果：
...
在后面的[external](external#tips--tricks)部分也记录了`external`功能和这个技巧的用法。 这是一个很好的方法，可以让你开始编写一些 ReScript 代码，而不必担心与某个特定库的绑定关系是否存在。
