...
记录和 JavaScript 中的object类似，但有如下特点：
- 默认是不可变的
- 成员字段是固定的(无法扩展)
...
-->
上面3个定义在告诉我们“所定义的记录的类型定义在 School 文件中”。 第一个，即常规的类型注释是首选。
...
ReScript的记录会直接编译为 JavaScript 的对象；可以查看上面的 JS 编译结果。
...
当直接在值上进行匹配的时候，其被看作是一个 option 值。例如：
...
类型系统会抱怨`me`是一个`person`类型，而且`getAge`只对`monster`有效。如果你需要这样的能力，请使用 ReScript 中的[对象](object.md)描述。
...
在[`v10`](/blog/release-10-0-0#experimental-optional-record-fields)中引入的多可选记录字段就是为了帮助解决这个问题。 多可选字段将使你不必指定所有字段，而只需指定你所关心的字段。对于绑定和其他具有大型配置对象的 API 来说，这是对“人体工程学”的重大改进。
