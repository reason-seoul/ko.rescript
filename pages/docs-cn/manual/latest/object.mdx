...
尽管 ReScript 记录可以编译成干净的 JavaScript 对象，但 ReScript 对象是模拟/绑定到 JS 对象的更好的候选者，你马上就会看到。
...
声明是**可选的**，这与记录不同。对象的类型是从值中推断出来的，所以你从来不需要真正写下它的类型定义。尽管如此，这里是它的类型声明语法：
...
**注意**：如上所述，与记录不同的是，这个 `me` 值**不会**试图找到一个符合 `"age"` 和 `"name"` 字段的类型声明；相反，`"me"` 的类型被推断为 `"{"age": int, "name": string}"`。这很方便，但也意味着下面的代码无错通过了类型检查：
...
`external` 特性和上面技巧的用法记录在后面的 [external](external#tips--tricks) 章节。这是一个很好的方法，可以开始编写一些 ReScript 代码，而不必担心是否存在与某个特定库的绑定关系。
