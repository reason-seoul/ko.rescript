---
title: 元素和JSX
description: "React元素的基本概念以及如何在JSX中使用它们"
...
React元素是React应用的最小构件。本页将解释如何使用我们专用的 JSX 语法在 React 应用程序中处理 `React.element`。
...
> **注意：** 本页假设你的 `bsconfig.json` 拥有配置 `"reason": { "react-jsx": 3 }`，否则你的JSX将不会转换为它的React特定形式。
...
绑定 `element` 和表达式 `{React.string("Hello World")}` 都是 `React.element`类型，这是在React应用中表示React元素的基本类型。元素描述了当你将应用渲染到DOM时在屏幕上看到的内容。
...
假设您想创建一个函数来处理另一个 React元素，比如 `children` ，你可以将其标记为 `React.element` ：
...
理解 `React.element` 的定义非常重要，因为它在React API中被大量使用，例如 `ReactDOM.render(element, ...)` 等。请注意，JSX不会自动将 `string` 转换为 `React.element` （ReScript强制要求显式类型转换）。例如， `<div> Hello World </div>` 将不会进行类型检查（这实际上是好事，因为它是细微BUG的一个巨大来源！），你需要使用 `React.string` 函数转换 `"Hello World"`。

<!-- Fortunately our React bindings bring all necessary functionality to represent all relevant data types as `React.element`s. -->
幸运的是，我们的 React 绑定拥有所有必要的功能，用于将所有相关的数据类型表示为 `React.element` 。

<!-- ## Using Elements within JSX -->
## 在JSX中使用元素

<!-- You can compose elements into more complex structures by using JSX: -->
使用JSX，你能将元素组合成更复杂的结构：
...
有时，当你与现有的ReactJS代码库进行大量互操作时，你会遇到因为语法限制而不能使用JSX语法的情况。查看[逃生舱口](#escape-hatches)章节来找到解决办法。
...
除了使用JSX来创建我们的React元素或React组件，`React` 模块还提供了各种函数来从原始数据类型创建元素:
...
它还提供了 `React.array` 来将多个元素表示为单个元素（对于渲染数据列表和传递子元素很有用）：
...
**注意：** 我们不提供 `React.list` 函数，因为 `list` 值会增加运行时开销。ReScript关注干净、惯用的JS输出。如果你想把一个元素 `list` 转换成单个React元素，使用 `Belt.List.toArray` 转换为数组后再转换为 `React.array`。
...
由于ReScript的强类型特性，它不允许 `element || null` 约束。当你要表达一个值可能被渲染或可能不被渲染，你需要 `React.null` 常量来表示 *Null*
...
**注意：** 本章节介绍了JSX使用的底层API，只有在遇到某些JSX语法限制时才应使用。关于JSX内部实现的更多信息可参考[Beyond JSX](./beyond-jsx)。
...
**注意：** 组件和属性的细节会在[下一章](./components-and-props)说明。
...
这个特性通常在与现有的JS/ReactJS代码交互时使用。在纯ReScript React应用中，你会传递一个渲染函数（也叫“渲染属性”）：
...
**注意：** 这里我们传递了一个属性 `"children": React.null` 来满足类型检查。React会忽略children属性而不是children数组。
...
这个函数主要被我们的JSX转换使用，通常你需要使用 `React.createElement` 并传递一个children属性。
...
要创建DOM元素（ `<div>`，`<span>` 等），使用 `ReactDOMRe.createDOMElementVariadic` ：
...
上面的函数需要 `ReactDOM.domProps` 构造函数，这样ReScript可以确保我们只传递有效的DOM属性。你可以在[ReactDOM](https://github.com/reasonml/reason-react/blob/master/src/ReactDOM.re#L61)模块中找到所有可用属性的列表。
...
**注意：** 这是一个逃生舱口特性，只有在与现有的JS代码/库进行交互时才有用。
...
有时需要复制一个现有元素，并对新实例设置、覆盖或添加属性值，或者你想要设置无效的属性名，例如 `data-name`。你可以使用 `React.cloneElement`：
...
上面提到的特性也可以模仿 `属性传播`，一个在ReactJS代码库中常见的用法，但因为它的不安全性和不正确性，我们非常不推荐你这样做（例如，给组件添加未定义的额外属性是没有意义的，并导致很难找到BUG）。
...
在ReScript中，我们会将需要的属性显式传递给叶子组件，或者使用renderProp。我们引入了[JSX双关](/docs/manual/latest/jsx#punning)语法来让属性传递过程更便捷。
