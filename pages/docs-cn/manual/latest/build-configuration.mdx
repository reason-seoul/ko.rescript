...
`bsconfig.json` 是 `rescript` 唯一需要的，并且是强制性的构建元文件。
...
**解释**：默认情况下，你的文件一旦被用作第三方依赖，用户就可以在全局范围内使用它。例如，如果你有一个 `Util.re`，而你的库的使用者也有一个同名的文件，它们会发生冲突。开启 `namespace` 可以避免这种情况，命名空间为你项目的所有文件提供一个额外的模块包装层；而不是一个全局的 `Util` 模块，用户将看到的是 `MyProject.Util`。**命名空间影响的是你的用户，而不是你自己**。
...
也就是说，在 ReScript 中，“命名空间”只是一个花哨的术语，指的是自动生成的模块，它包装了你项目的所有文件（当然是又快又好的！）供第三方使用。
...
我们不为你的项目创建文件夹级别的命名空间；你的所有文件名必须是唯一的。这是一个约束条件，基于它可以实现一些特性，如快速搜索和更容易重新组织项目。
...
**注意**：`bsconfig.json` 的 `name` 应与 `package.json` 的 `name` 相同，以避免出现混乱的边界情况。这就意味着你不能使用驼峰命名的名字，如 `MyProject`，因为 `package.json` 和 npm 禁止你这样做（因为有些文件系统不区分大小写）。要想让命名空间/模块的名字是 `MyProject`，可以写上 `"name": "my-project"`。ReScript 会正确地将它转化为驼峰命名名称。
...
你可以把你的目录标记为仅用于开发（dev-only）（例如测试文件）。这些文件不会被构建然后暴露给第三方，甚至不会暴露给同一项目中的其他“开发”目录。
...
它们是 ReScript 的依赖项列表，就像 `package.json` 中的依赖项一样，ReScript 将在 `node_modules` 中搜索它们。
...
**自 8.4 版本起可用**，它是版本锁定依赖项的列表。每当你用 `rescript` 构建一个顶级包（例如你的主程序）时，版本锁定依赖总会被重新构建。
...
这对于同时处理多个独立的 ReScript 包非常有用。更多的使用细节可以在专门的[版本锁定依赖](./build-pinned-dependencies)页面找到。
...
**自 9.0 版本起可用**：此设置允许依赖外部构建的 stdlib 包（而不是本地构建的 stdlib 运行时）。这对转移只在 JS 或 TS 中使用的包很有用，这样对 ReScript 开发工具链没有任何依赖。

<!--
More details can be found on our [external stdlib](./build-external-stdlib) page.
-->

更多细节可在我们的[外部标准库](./build-external-stdlib)页面找到。
...
- `"module": "es6-global"` 使用相对路径解析 `node_modules`。这有利于 ES6 在开发阶段的使用，并与与 Safari 和 Firefox 等支持 ES6 模块的浏览器相结合。**不会有开发时的捆绑**！
- `"in-source": true` 表示输出到源文件同一位置。如果你省略它，生成的构建工件会放到 `lib/js` 中。除了这两种就没有其他的配置选项了
...
只有你在开发项目时这个配置才对你适用。当项目被用作第三方库时，用户自己的 `bsconfig.json` `package-specs` 会在逻辑上覆盖这里的配置。
...
- 它避免了为 ReScript 文件使用构建系统加载器的需要。这种方式加上 `in-source` 构建意味着，在**基本不碰你的 JS 构建流水线**的情况下，就能将 ReScript 项目集成到你的纯 JS 代码库中
- [genType](/docs/gentype/latest/introduction) 要求文件以 `bs.js` 为后缀来编译 JS 工件。如果你正在使用 `genType`，你需要暂时使用 `bs.js`
...
上面的配置关闭警告 `44` 和 `102`（多态比较），将警告 `5`（部分应用的结果具有函数类型并被忽略）变成硬性错误。
...
当警告被触发时，警告号码会显示在构建输出中。参见[警告号码](./warning-numbers)获取完整的列表。
