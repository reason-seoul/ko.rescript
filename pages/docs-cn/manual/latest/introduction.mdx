...
ReScript看起来像JS，运行起来像JS，并且可以编译到简洁、可读且高效的高质量 JS 代码，还可以直接在浏览器和 node 中运行。
...
**这意味着你可以随时使用ReScript，访问庞大的 JavaScript 生态，就好像已经用了很久 ReScript 一样**
...
我们非常尊重TypeScript，我们认为 TypeScript 为JavaScript生态起到了积极作用。ReScript与 TypeScript 有一些相同的目标，但在一些重要的细节上又有足够的不同。
...
- TypeScript的目标是在覆盖整个 JavaScript 语言特性的基础上提供更多的特性。**ReScript只覆盖了一个精心挑选的 JavaScript 子集**。 比如说：我们选择 _数据+函数_ 的抽象方式而不是class；使用简洁的模式匹配而不是难以维护的`if`判断和动态分发；使用更好的数据模型而不是字符串区分。
...
- 因此，TypeScript的类型系统必然是复杂、充满陷阱、需要反复微调、有时甚至很慢，并且需要使用包含很多噪音的类型签名，这些签名通常给人的感觉更像是流水账，而不是清晰的文档。相反，ReScript的类型系统有如下特点：
  ...
  - Is the same for everyone。 No knobs，no bikeshedding opportunity。
  - 归功于语言的简洁性和精心选择的 JS 子集，ReScript运行性能极佳，是 JavaScript 开发中最快的编译器&构建系统工具链之一。
...
- 如果说迁移到 TypeScript 是一个"广度优先"的过程，而迁移到 ReScript 就是一个"深度优先"的过程。你可以通过在你的代码库中"开启"TypeScript完成迁移，这里开一点那里开一点。但这样可以带来多大的类型安全收益呢? 如何具体评估? 类型错误任然可能通过已经转换到 TS 的代码传播。但是对于ReScript，互操作特性划出了明确的界限：这里是 ReScript 代码，这里是 JS 互操作代码。迁移完成的 ReScript 代码都是100%类型安全的。你可以通过逐个文件的将 JS 转换到ReScript，每次转换都会提升安全性。

<!-- \* When you absolutely need to write or interoperate with free-for-all JavaScript, we expose enough escape hatches for you. -->

\* 当你有编写 JavaScript 或与之进行互操作的必要时，我们为您敞开了“逃生通道”。

<!-- ## Other Highlights -->
## 其他亮点
<!-- Aside from the aforementioned simple, robust and fast type system, ReScript presents a few more advantages. -->

除了先前提到的简洁、鲁棒且快速的类型系统，ReScript还有一些其他优势。
...
天才工程师对 JavaScript 进行激进的优化已经持续很长一段时间了。但不幸的是，即使是经验丰富的 JS 开发者，也很难知道如何正确地利用 JS 的性能。ReScript的类型系统和编译器在编写默认情况可以很自然的引导你编写性能良好的代码，充分利用各种 JIT 优化(hidden classes，inline caching，avoiding deopts，etc)
...
编写高效的 JavaScript 代码的一个广为流传的格言是：写的时候假装它有一个类型系统(为了触发 JS 引擎的启发式优化); ReScript给你一个真正的类型系统，并在默认设置下生成有利于优化的代码。
...
- 在 global 级别，ReScript在运行完其自身的死代码消除 pass 后，继续整合像[Rollup](https://github.com/rollup/rollup) and [Closure Compiler](https://developers.google.com/closure/compiler/)这样的工具，生成死代码消除友好的代码。
- 这些优点也适用于 ReScript 自身的运行时(同样用 ReScript 编写)
...
ReScript的`Hello World`程序只会生成**20字节**的 JavaScript 代码。此外，标准库中代码会在你需要的时候被包含进来。
...
ReScript项目的构建时间比其他替代方案要快**一到两个数量级**。在 watcher 模式中，构建系统经常在你从编辑器切换到终端前就完成了构建(大约几十毫秒)。快速的迭代周期减少人们保持自己的心理状态的需要; 这也使人们可以更频繁更长时间的保持专注状态。
...
其他编译到 JS 的语言生成不可读的 JavaScript 代码使得它们：
- 难以调试(模糊的堆跟踪，混乱的变量名)
...
ReScript生成的 JS 代码可读性很好，这对于通过理解代码的编译结果来学习 ReScript 和漏洞审查是十分重要的。
...
这种特性与功能完善的 JS 互操作系统相结合，允许将 ReScript 代码插入到现有的JavaScript代码库中，几乎不会被注意到。
...
ReScript将一个源文件映射到一个 JavaScript 输出文件，这简化了现有工具(如打包工具和测试运行器)的集成。你甚至可以开始编写单个源文件，而无需对构建设置进行太多更改。每个文件的代码结构也大致保留了下来。
...
我们希望上面的内容能够让你了解到 ReScript 的思想和与其他语言的差异。请随时尝试我们的[在线编译工具](/try)来感受ReScrit！
