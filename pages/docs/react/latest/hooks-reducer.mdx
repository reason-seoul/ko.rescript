...
**注意**： 你会发现，归功于 [不可变记录](/docs/manual/latest/record)，[变体](/docs/manual/latest/variant) 和 [模式匹配](/docs/manual/latest/pattern-matching-destructuring) 特性，action / reducer 模式可以在 ReScript 中轻松地表达 action 和 state 转换。
...
你可以充分利用变体的强大功能，通过数据 payload 来表达 action，从而参数化 state 转换：
...
你可以选择惰性地创建 `initialState`。为此，需要将 `init` 函数作为 `React.useReducerWithMapState` 的第三个参数传入，这样初始 state 将被设置为 `init(initialState)`。
