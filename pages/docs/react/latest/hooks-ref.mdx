...
`React.useRef` 返回一个可变的 ref 对象，它的 `.current` 记录字段被初始化为传递的参数（`initialValue`）。返回的对象将持续到组件的整个生命周期结束。

<!-- Essentially, a `React.ref` is like a "box" that can hold a mutable value in its `.current` record field. -->
实质上，`React.ref` 就像的一个”盒子“，它将可变值保存在 `.current` 记录字段。
...
作为访问 DOM 的主要方式，你可能对 refs 很熟悉。如果你通过 `<div ref={ReactDOM.Ref.domRef(myRef)} />` 将一个 ref 对象传给 React，当相应的 DOM 节点发生改变的时候， React 会将 `.current` 属性设置为该节点。

<!-- However, `useRef()` is useful for more than the ref attribute. It's handy for keeping any mutable value around similar to how you’d use instance fields in classes. -->
然而，`useRef()` 不仅适用于 ref 属性。它可以方便地保存可变值，类似于在类中使用实例字段的方式。

<!-- This works because `useRef()` creates a plain JavaScript object. The only difference between `useRef()` and creating a `{current: ...}` object yourself is that useRef will give you the same ref object on every render. -->
这是因为 `useRef()` 创建了一个普通 JavaScript 对象。使用 `useRef()` 和自己创建一个 `{current: ...}` 对象的区别是，useRef 会在每次渲染时给你相同的 ref 对象。
...
使用 [Refs & DOM](./refs-and-the-dom#回调-refs) 章节的例子：
