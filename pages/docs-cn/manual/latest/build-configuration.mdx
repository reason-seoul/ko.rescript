...
也就是，在ReScript中，"命名空间 "只是一个花哨的术语，指的是自动生成的模块，它包装了你项目的所有文件（当然是有效和正确的！）供第三方使用。
...
**注意**：`bsconfig.json` 的`name`应与`package.json`的`name`相同，以避免混乱的角落情况。然而，这意味着你不能使用驼峰命名的名字，如`MyProject`，因为`package.json`和npm禁止你这样做（有些文件系统是不区分大小写的）。要将名字空间/模块作为`MyProject`，请写上`"name": "my-project"`。ReScript将正确地将其转化为camelCased名称。
...
**从9.0开始**：此设置允许依赖外部构建的stdlib包（而不是本地构建的stdlib运行时）。这对转移只在JS或TS中使用的包很有用，而对ReScript开发工具链没有任何依赖性。
...
这是每次文件被重新编译时都会调用钩子。这对JS构建系统的互操作有好处，但请**谨慎**使用。为每个重新编译的文件调用你的自定义命令会降低你的构建速度，甚至会使你的lib的第三方用户的构建体验恶化。
...
- `myCommand`只是作为`myCommand`被调用，也就是一个全局可用的可执行文件。但要注意的是，ReScript并不读取你的shell环境，所以如果你输入例如`node`，除非你指定一个绝对路径，否则它不会找到它。或者，在你的脚本顶部添加`#!/usr/local/bin/node`来直接调用它，而不需要在前面加上`node`。
...
- `"模块": "es6-global"`使用相对路径解析`node_modules`。这有利于ES6在开发过程中与Safari和Firefox等目前支持ES6模块的浏览器结合使用。**没有和开发捆绑**!
