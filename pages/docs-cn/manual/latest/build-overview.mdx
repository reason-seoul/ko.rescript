...
ReScript 自带一个快速且简洁的构建系统[`rescript`](https://www.npmjs.com/package/rescript)，并被用作社区的权威构建系统。
...
这是 `rescript build` 命令的一个别名。

<!--
To keep a build watcher, run:
-->

要保留一个构建监听（watcher），请运行：
...
**注意**：第三方库（指在 `node_modules` 中，或通过 `pinned-dependencies` 声明）不会被监听，因为这样做可能超过node.js的监听数量限制。
