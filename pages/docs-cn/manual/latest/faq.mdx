...
此外，ReScript 还提供了自己的语法、构建系统、IDE、后端、JS 互操作、额外的语言功能等。
...
对于大多数 ReScript 用户来说，他们不需要学习 OCaml 或使用 OCaml 工具链就可以从 ReScript 中受益。
...
参考[这里](/blog/bucklescript-is-rebranding)。Reason 是 OCaml 的一个语法层，BuckleScript 也采用了这个语法层。目前的 ReScript 编译器也支持旧的 Reason v3.6 语法，以便向后兼容。我们将长期支持它，以确保现有用户不会得到破坏性的改变。
...
ReScript 团队不再使用旧的 Discord，我们鼓励您将问题移至论坛。
