...
**注意**：惰性值是**不是**[共享数据类型](shared-data-types.md). 不要在你的 JavaScript 代码中依赖它的运行时表示。
...
你也可以使用[模式匹配](pattern-matching-destructuring.md)来触发计算：
...
我们的文件读取例子可能因为 `readdirSync` 触发异常。下面是处理方式：
