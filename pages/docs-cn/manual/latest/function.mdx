...
声明中的 `~x` 部分意味着该函数接受一个标签为 `x` 的参数，并且可以在函数体中用相同的名称来指代它。为了简洁起见，你也可以在函数体中用不同的名字指代参数：
...
带标签的函数参数可以在声明时设置成可选项。然后你可以在调用函数时省略它们。
...
**注意**，因为类型系统的缘故，只要你有一个可选参数，你就需要确保在它后面至少有一个位置参数（positional argument，又称非标记的，非可选的参数）。如果没有，可以提供一个哑的 `unit`（`()`）参数。
...
当涉及到签名和类型标注时，带有可选标记参数的函数可能会令人困惑。事实上，一个可选标记参数的类型在不同地方看起来是不同的，这取决于你是在函数的调用处，还是在函数体内部。在函数外部，一个原始值要么被传入（比如 `int`）要么被舍弃不传入。在函数内部，参数总是存在的，但其值是一个 `option` 值（`option<int>`）。这意味着类型签名是不同的，这取决于你写的是函数类型还是参数类型的标注，前者是一个原始值，而后者是一个 `option`。
...
因此如果你在编写带有可选标记参数的函数的签名时碰巧遇到了困难，请记住参数类型内外有别这一点！
...
这样做的意思是“我知道 `radius` 是可选值，当我给它传递一个值时，它需要是一个 `int`，但我不确定我传递的值是 `None` 还是 `Some(val)`，所以我给你传递整个 `option` 包装”。
...
ReScript 的函数是默认柯里化的，这是少数的编译到 JS 可能会付出性能代价的情况之一。编译器会尽最大可能地去除这些柯里化。然而，在一些边缘情况下，你可能希望保证没有柯里化。这时可以在函数的参数列表前放一个点：
...
错误处理是通过简单地使用 `try/catch` 或者带 `exception` 情况的 switch 来完成的，就像在非异步的函数中一样。JS 异常和 ReScript 中定义的异常都可被捕获。编译器负责将 JS 异常打包成内置的 `JsError` 异常：
...
偶尔你可能想忽略一个函数的返回值，ReScript 提供了一个 `ignore()` 函数，它抛弃参数的值并返回 `()`：
