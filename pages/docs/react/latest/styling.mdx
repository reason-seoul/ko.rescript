...
它是一个使用 [标签参数](/docs/manual/latest/function#labeled-arguments) 的函数调用（因此是类型化的），参数映射到我们熟悉的 CSS 样式对象 `{color: '#444444', fontSize: '68px'}`。对于 CSS 规范中的每个 CSS 属性，在 `make` 函数中都有一个驼峰命名的标签参数。
