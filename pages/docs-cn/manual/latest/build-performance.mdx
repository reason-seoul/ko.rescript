...
有的时候由于一些混乱的基础设置，你的构建过程会很慢。通过 `bstracing`，我们为你的构建提供了一个交互可视化工具：
...
`rescript` 本身负责一些杂七杂八的任务还有监听器的 Node.js 封装。底层的、无监听的且快速的原生 `rescript` 被称为 `rescript.exe`。它位于 `node_modules/rescript/{your-platform}/rescript.exe`。
...
如果你不需要监听，你可以运行上面的 `rescript.exe`。这就避免了 Node.js 冗长的启动时间，它的启动时间可能在 `100ms` 左右。我们的编辑器插件会找到并使用这个本地的 `rescript.exe` 以获得更好的性能。
...
基于小型项目构建的原始 `rescript.exe` 的构建时间应该在 `70ms` 左右。当你使用附带监视器的 JS `rescript` 的包裹器时，这个速度会翻倍，因为你不需要在每次修改时手动运行构建（尽管你应该选择原始 `rescript.exe` 用于编程用途，例如将 rescript 插入到你现有的 JS 构建流水线）。
...
在 Retina Macbook Pro Early 2015（3.1 GHz Intel Core i7）上，我们使用 https://github.com/rescript-lang/build-benchmark ，对一个拥有 10,000 个文件（2 个目录，每个 5000 个文件，前 5000 个没有依赖项，后 5000 个有 10 个依赖前一个目录的文件）的大型项目对 `rescript.exe` 进行了压力测试。
...
`rescript` 是基于文件的构建系统。尽管在内存中构建会极大的提升构建速度，但是我们选择不在内存中构建。在内存中构建需要冒着内存泄漏、内存耗尽、构建半途中断以及其他风险。我们的监听模式可以整天整月的开着，不会出现内存泄漏。
...
简而言之，得益于我们的编译器和构建系统架构，我们能够**只构建需要构建的内容**。例如，如果 `MyFile.res` 没有改变，那么它就不会被重新编译。你可以在 JavaScript 等语言中粗略地模仿这种渐进方式，但不幸的是，其正确程度很低。例如，如果你重命名或移动一个 JS 文件，那么监听器可能会感到困惑，就无法找到“新”文件，或不能正确地清理内容，导致你需要清理你的构建并重新构建一次，这就没有达到增量的目的。
...
ReScript 通过使用接口文件（`.resi`）（等同于[模块签名](module.md#signatures)），只向外暴露出你需要的东西，这自然会加快增量构建的速度。例如，如果你改变了一个 `.res` 文件，而其对应的 `.resi` 文件没有暴露所改变的部分，那么你就减少了你必须重构建的依赖文件的数量。
...
- 谨慎引入更多的依赖项：库、语法转换（例如非官方支持的 PPX）、构建步骤加载器等。与提供的 API 效益相比，它们更可能会拖累编辑和构建的体验
