...
在 9.0 及更新的版本中已经不再使用，请在侧边栏菜单中切换至旧版本的文档进行查看。
...
- 自动生成函数用于将值在 ReScript 内部与 JS 运行时之间转换
- 将记录类型转换为有自动生成的创建、访问和方法函数的某一抽象类型
- 生成一些辅助函数，如一些与 record 中属性名相关的函数
...
**注意：**大量使用代码生成可能会让你的代码难于理解，请悉知（因为生成的代码在源代码中不可见，而且你只需要知道装饰器生成的函数/值的种类）。
...
使用 `@deriving(accessors)` 为一个变体类型创建被造器使用的访问函数：
...
具有载荷的变体构造会生成函数，而无载荷构造器则会生成值（变体的内部表示）。
...
**注意**:
- 生成的访问器名字是小写
- 虽然可以在 JavaScript 端使用这些生成的辅助函数，但请不要依赖它们的实际值！
...
- 当你希望在 JS 端不透明地使用这些值或函数时并传回一个 variant 构造器时 (因为 JS 中没有类似的东西)
...
请注意，有些时候如果你只是想_传入载荷到一个构造器_，那么你不需要生成一个函数来做这件事。可以直接使用 `->` 语法，例如 `"test"->Submit`。
...
使用 `@deriving(accessors)` 为一个记录类型创建字段名访问器。
...
可以为一个变体类型添加 `@deriving(jsConverter)` 来创建转换器函数。这些函数可以对 ReScript 中的变体值与 JS 中的整数值与枚举值进行双向转换。
...
因为要进行双向转换，`jsConverter` 会生成以下类型的函数：
...
`fruitFromJs` 的返回类型是 `option`，因为不会每个整数都可以映射到一个构造器。

<!--
You can also attach a `@as(1234)` to each constructor to customize their output.
-->

也可以为 variant 中的构造器添加 `@as(1234)` 来自定义其输出。
...
**注意**：因为这里使用了 `@as`，数字编号会发生变化。`Apple` 还是 `0`，但 `Orange` 变成了 `10`，`Kiwi` 变成了 `100`，因此 `Watermelon` 是 **`101`**！
...
与 JS 对象和记录类型间的转换类似，通过使用 `@deriving(jsConverter)` 及其选项 `newType`，你可以隐藏 JS 的 enum 是整数的事实：
...
这个选项会导致 `@deriving(jsConverter)` 生成以下类型的函数：
...
这次，`fruitFromJs` 返回的值与之前的非抽象类型 fruit 不同，它不包含 `option`。因为传递给它错误值是不可行的，只有 `fruitToJs` 可以创建 `abs_fruit` 类型的值。
...
给记录类型添加 `@deriving(abstract)` 可以将它扩展为一个构造函数，并为其字段和方法（methods）生成 getter/setter 函数。
...
通常，我们会想将 ReScript 的记录类型编译为具有相同结构的 JS 对象。但在某些特定场景中 `@deriving(abstract)` 还是很有用，比如你需要将一个带有 `option` 类型字段的记录编译到 JS 对象，但是在 JS 中这个属性值为 `undefined` 时属性不应该出现（例如：`{name: "Carl", age: undefined}` 与 `{name: "Carl"}`）。请查看[可选标签（Optional Labels）](#optional-labels)一节来了解关于该场景的更多细节。
...
**注意**：`person` 类型**不是**一个记录！它是一个看起来像记录的类型，使用了记录的语法和类型检查。`@deriving(abstract)` 装饰器会把它转换为一个“抽象类型”（我们无法知道抽象类型的实际值是什么）。
...
绑定一个来自 JS 端的 `person` 对象不是必须的，因为我们可以在 ReScript 端创建这样的 `person` JS 对象。
...
由于 `@deriving(abstract)` 会把 `person` 记录转换为一个抽象类型，因此这里无法像往常一样直接创建一个 `person` 记录。`{name: "Joe", age: 20, job: "teacher"}` 在这种情况下是无用的。
...
相应地，可以使用 `@deriving(abstract)` 隐式生成的与记录类型同名的**创建函数**来解决这个问题：
...
有些时候，会需要绑定某些字段名在 ReScript 中无效的 JS 对象。例如 `{type: "foo"}` （type 在 ReScript 中是保留关键字）和 `{"aria-checked": true}`。可以使用 `@as` 来规避这个问题：
...
未定义的可选值不会作为属性出现在最终的 JS 对象中。上面的例子中可以看到 `name` 字段被忽略了。

<!--
**Note** that the `@optional` tag turned the `name` field optional. Merely typing `name` as `option<string>` wouldn't work.
-->

**注意**，`@optional` 标签把 `name` 字段标记为可选。只将 `name` 的类型标注为 `option<string>` 做不到这样。
...
**注意**：因为创建函数包含可选字段，所以需要在最后使用 `()` 以表示[已完成该函数的应用](function#optional-labeled-arguments)。
...
再次强调，由于 `@deriving(abstract)` 隐藏了记录的实际结构，所以无法通过使用类似 `joe.age` 的方式访问字段。需要通过生成 getter 和 setters 来补救。
...
使用 `@deriving(abstract)` 的记录类型的每个字段都会生成一个对应的 getter 函数。以上面的代码为例，它会生成三个函数：`nameGet`、`ageGet` 和 `jobGet`。它们会接收一个 `person` 值作为参数，并分别返回 `string`、`int` 和 `string`。
...
另外，你可以使用 [管道](pipe) 操作符（`->`）来得到更好看的访问语法：
...
如果你想为 getter 函数使用更短的名字，可以使用 `light` 选项：
...
`@deriving(abstract)` 值默认是不可变的。如果想将其变为可变，需要将抽象记录的字段标记为 `mutable`，就像标记一个普通记录为可变一样：
...
你可以为类型附加任意的方法（事实上，不只是 `@deriving(abstract)` 后的记录类型，它可以是_任何_类型）。查看[对象方法（Object Method）](bind-to-js-function#object-method)页面中的 “绑定 JS 函数”小节来获取更多信息。
...
可以利用 `@deriving(abstract)` 来获取更细粒度的访问控制。
...
可以在实现文件（`.res`）中将字段标记为可变的，而在接口文件（`.resi`）中_隐藏_这种可变性：
...
好了！现在，你可以根据需要在自己的文件中使用可变性，并防止其他人这么做。
...
访问器依然存在，只是你无法再创建该数据结构。因此，它非常适合创建 JS 对象绑定的同时阻止其他人创建更多该对象！
...
很多时候多个抽象类型会拥有类似的属性。由于 ReScript 会在类型被定义的作用域中展开全部的抽象 getter，setter 和创建函数，你最终会遇到会名字遮蔽的问题。
...
要解决这个问题，可以使用模块将类型与其相关函数封装在一起，并在稍后通过使用局部 open 声明来使用它：
...
可以使用具有 `@obj` 的 `external` 绑定来创建一个函数。当调用该函数时，它会创建一个具有与该函数参数标签相对应字段的 JS 对象。
...
这样做非常方便，如果有一些不想被传递的参数，你可以将某些带标签的参数标记为可选的。这样生成的对象便不会包含这些值为空的可选字段。这样你就你可以在运行时动态创建只包含需要字段的对象。

<!--
For example, suppose you need a JavaScript object like this:
-->

举个例子，假设你需要像这样的 JavaScript 对象：
...
但只需要前三个字段。options 字段是可选的。因此，可以声明像这样的绑定函数：
...
**注意**: 由于语法限制，末尾的 ` = ""` 只是一个哑占位符。当前版本它没有任何实际意义。
...
这个函数有 4 个带标签的参数（第 4 个是可选的），和末尾的一个无标签的参数（参数 options 是一个[可选参数](function#optional-labeled-arguments)）。由于在 ReScript 中 `type` 是一个保留的关键字，所以参数 `\"type"` 必须带有双引号以[避免冲突](use-illegal-identifier-names)。
...
同样有趣的是其返回类型：`_`，它会让 ReScript 自动对 JS 对象的所有类型进行推导，这就避免了需要手写类型的麻烦！
