...
为了区分 JavaScript 异常和 ReScript 异常，ReScript 将 JS 异常命名为 `Js.Exn.Error(payload)` 变体。为了捕捉一个从JS端抛出的异常:
...
`raise(MyException)`引发一个ReScript异常。如果要引发一个JavaScript异常(不管你的目的是什么), 使用`Js.Exn.raiseError`.
...
## Catch ReScript Exceptions from JS | 从JS捕获ReScript抛出的异常
...
前面的部分没有你想象的那么有用；为了让你的JS代码与抛出异常的ReScript代码一起工作，后者实际上不需要抛出一个JS异常。ReScript的异常可以被JS代码所使用!
...
> 注意: `RE_EXN_ID`是一个用于记账的内部字段。不要在JS方面使用它。使用其他的字段。
...
上面的`BadArgument`异常需要一个内联记录类型。为了符合人体工程学，我们将该异常特例编译为`{RE_EXN_ID, myMessage, Error}`. 如果这个异常使用普通的位置参数，比如标准库的`Invalid_argument("Oops!")`, 它只使用一个参数，那么这个参数将被编译为JS的字段`_1`. 第二个位置参数将编译为`_2`, 等等。
...
### Catch Both ReScript and JS Exceptions in the Same `catch` Clause | 在一个 `catch` 子句中同时捕获ReScript和JS的异常
