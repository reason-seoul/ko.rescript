...
想要在 ReScript 中使用一些 HTML 语法吗？如果没有这个需求可以跳过这一章假装你什么也没看到！
...
ReScript 支持 JSX 语法，与 [ReactJS](https://facebook.github.io/react/docs/introducing-jsx.html) 中的语法相比有一些细微的差别。ReScript 的 JSX 并不与 ReactJS 绑定，它们会转化为正常的函数调用：
...
使用 [ReasonReact](https://reasonml.github.io/reason-react/en/) 的读者请**注意**：这并不是 ReasonReact 最终将 JSX 变成的东西，更多信息参见用法部分。
...
这是传递元素为 `child1` 和 `child2` 的列表到子位置的语法。它会被去糖化（desugar）为包含 `child1` 和 `child2` 的列表：
...
再次**注意**，这不是 ReasonReact 做的变换；ReasonReact 会将最终的列表变成一个数组，但是这样在思路上没问题。
...
所以很自然地，`<MyComponent> myChild </MyComponent>` 被去糖(desugar)为 `@JSX MyComponent.createElement(~children=list{myChild}, ())`。也就是说，无论你做什么，传递到子位置的参数都会被包裹在一个列表中。那如果你不希望这样呢？比如说**如果你想直接传递 `myChild` 而不需要额外的包装**呢？
...
这在你处理的 `myChild` 已经是一个列表的情况下是非常有用的，你可以直接转发而不需要额外包装它（这将产生一个类型错误）\*。它还允许你在 `children` 位置传递任意的数据结构（记住，JSX 的 `children` 实际上只是一个常规的 prop）。
...
参见 [ReasonReact JSX](https://reasonml.github.io/reason-react/docs/en/jsx) 了解 JSX 的应用实例，它将上面的转化为 ReasonReact 的特定调用。
...
- 没有对 JSX prop 展开的支持，比如 `<Comp {...props}}. />`。但我们有子节点展开语法：`<Comp> ...children </Comp>`
- 双关！（punning）
...
“双关”是指标签和值相同时的语法简写。例如，在 JavaScript 中，不用写 `return {name: name}`，可以用 `return {name}` 代替。
...
Reason 的 JSX 支持双关，`<input checked />` 只是 `<input checked=checked />` 的缩写。格式化工具将帮助你尽可能地按照双关语的语法进行格式化，这在有很多 prop 需要传递的情况下很方便：
...
这样一来，Reason JSX 组件可以在需要额外的库来避免传递 props 之前塞入更多的 props。
...
注意这和 ReactJS JSX 不同，ReactJS JSX 没有双关。它的 `<input checked />` 会去糖为 `<input checked=true />`，这是为了和 DOM 对的上，也是为了向后兼容。
...
JSX 调用也同样支持[标签参数](function.md#labeled-arguments)特性：可选参数，显式传递的可选参数和默认的可选参数。
...
\* 你可能会想，为什么在 ReactJS 中从来不需要这样的子节点展开呢；ReactJS 使用了一些特殊的运行时逻辑 + 特殊的语法转换 + 变量参数检测和标记来避免大多数的这些情况（[看这里](https://github.com/facebook/react/blob/9b36df86c6ccecb73ca44899386e6a72a83ad445/packages/react/src/ReactElement.js#L207)）。这样的动态使用为类型系统带来了_巨量_的复杂度。由于我们控制了整个语法和 ReasonReact，我们决定引入子节点展开来区分包装和不包装的情况，这样就没有编译时和运行时开销了！
