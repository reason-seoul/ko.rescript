...
之后的 Uglifyjs 优化过程会将整个 `if` 块移除。这样可以做到在开发模式使用大量开发时警报代码的同时确保压缩（精简）后的生产代码能去除掉这些昂贵的代码，ReactJS 等项目就是这样做的。

<!--
So, in ReScript, producing that example `if (process.env.mode === 'development')` output is important. This first try doesn't work:
-->

所以，在 ReScript 中，生成类似 `if (process.env.mode === 'development')` 的输出非常重要。以下的尝试没有效果：
...
现在生成的 JS 代码可以像其他的 JS 代码一样通过 Webpack 和 Uglifyjs，并且 `console.log` 代码块能被去除。

<!--
The inlining currently only works for **string, float and boolean**.
-->

内联目前只适用于 **string，float 和 boolean**。
...
这**不是**优化。这是一个边缘特性，提供给需要为 JavaScript 后续步骤（例如条件编译）内联特定值的人们。除了条件编译输出的代码可能有所不同之外，内联和不内联简单值对于 JavaScript 引擎来说没有性能上的差异。
