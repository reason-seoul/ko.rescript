...
> **注意: **在以前的版本 (< 8.3) 中, 我们所有的互操作相关属性都以`bs.`前缀开始(`bs.module`, `bs.val`). 我们的格式化器将在较新的ReScript版本中自动删除它们.
