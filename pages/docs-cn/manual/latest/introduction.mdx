...
- TypeScript （诚然高贵）的目标是在覆盖整个 JavaScript 语言特性的基础上提供更多的特性。**ReScript只覆盖了一个精心挑选的 JavaScript 子集**。 比方说我们强调简单的 _数据+函数_ 而不是类；简洁的[模式匹配](/docs-cn/manual/latest/pattern-matching-destructuring.md)而不是脆弱的的 `if` 和动态分发；[合适的数据模型](/docs-cn/manual/latest/variant.md)而不是滥用字符串，等等。JavaScript 的超集只会越来越大；而 ReScript 不会 \*
