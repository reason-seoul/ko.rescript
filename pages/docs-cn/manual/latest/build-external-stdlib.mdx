...
你的ReScript项目依赖于`rescript`包作为一个[`devDependency`](https://docs.npmjs.com/specifying-dependencies-and-devdependencies-in-a-package-json-file)，其中包括我们的编译器、构建系统和运行时，如`Belt`。然而，如果你发布你的代码，你不得不把它移到`package.json`中的`dependency`。
- 到Docker或其他低存储量的部署设备。
...
**这是一个高级功能**，请只在上述情况下使用它，如果你已经使用了具有消除死代码的JS捆绑器，你可能不需要这个功能。

## Configuration | 配置

Say you want to publish a JS-only ReScript 9.0 library. Install the packages like this:

假设你想发布一个纯JS的ReScript 9.0库，可以像这样安装软件包：
...
现在编译的JS代码将使用`external-stdlib`定义的路径导入。检查JS输出标签。
...
**确保`bs-platform`和`@rescript/std`的版本号在你的`package.json`中匹配**，以避免由于不匹配的stdlib假设而遇到运行时问题。
