...
甚至JavaScript也有析构功能, 也就是"打开"一个数据结构, 提取我们想要的部分, 并为其分配到相应的变量名.
...
但是数组的例子是**高度不推荐的**(最好用元组代替), 因为完整性的原因, 列表的例子会让你出错. 你即将在后面看到, 使用数组和列表的正确方法是使用`switch`. 

## `switch` Based on Shape of Data | 基于数据的形状`switch`
...
在其他语言中, 你可能会用一系列`if-else`来实现上述功能, 可读性不好并且很容易出错. 在ReScrit中, 你可以使用强大的`switch`的模式匹配功能析构值, 然后基于析构的结果调用需要的代码块. 
...
- 如果该学生的gpa为0, 并且正在休假或公休, 则显示一个不同的信息.
- 对一个学生显式一条通用的信息.
...
### Ignore Part of a Value | 忽略某部分的值
...
**注意**: 你只能传递字面值(即具体的值)作为模式, 不能传递let-binding名称或其他东西, 下面的方法不能像预期的那样工作.
...
第一次使用ReScript的人可能会不小心写下这段代码, 认为当第二个值与`centerY`相同的时候会匹配`coordinates`. 实际上, 这会被解释为在`coordinates`上进行匹配, 并将元组的第二个值分配给`centerY`, 而这并不是我们想要的结果.

## Exhaustiveness Check | 穷举检查
...
如果上述特性还不够, ReScript还提供了可以说是最重要的模式匹配功能. **编译期检查丢失的模式**.
...
你看到我们移除的部分了吗? 这一次, 我们省略了对`person1`是`Teacher({name})`的情况的处理, 当`name`不是Mary或Joe.
...
如果你不处理`None`的情况, 编译器会发出警告. 在你的代码中不再有 `undefined` 的错误了!

## Conclusion & Tips & Tricks | 结论 & 技巧 & 诀窍
...
希望已经你看到模式匹配是如何通过简洁的析构语法, `switch`的适当条件处理和静态穷举检查改变编写正确代码的游戏规则的. 
...
**尽可能扁平化你的模式匹配**. 这是一个真正的消除错误的工具, 这里有一系列的例子, 从最差到最好.
...
这缺失更简洁了, 但会破坏上面提到的穷举检查; 尽量不要使用这个. 这是最好的写法:
...
这个时候, 要想在这段代码中犯错是非常困难的! 当你想使用一个有许多分支的if-else时, 最好用模式匹配来代替. 它更简洁, 而且[性能更好](variant#design-decisions). 
