...
上面的代码中引入了`Student.res`中的`message`. 每个ReScript文件同时也是一个模块, 所以获取其他文件的内容和获取其他模块的内容是一样的！
...
默认情况下, 每个文件的类型声明, 绑定和模块都会导出, 也就是可以由另一个文件直接使用. **这也就意味着, 这些值一旦被编译到JS就可以直接被JS代码所使用的**
...
## Work with JavaScript Import & Export | 和JavaScript的导入&导出一起使用
...
如果想了解如何导入JS模块以及如何导出内容给JS使用, 可以参考JavaScript互操作部分的[Import from/Export to JS](import-from-export-to-js.md)