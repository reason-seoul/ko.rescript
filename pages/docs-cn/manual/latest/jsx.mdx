...
你是否想要在 ReScript 中使用一些HTML语法? 如果不需要，可跳过这些假装你什么也没看到!
...
这是传递两个列表项目`child1`和`child2`到 children 位置的语法。它解构为一个包含`child1`和`child2`的列表。
...
再次**注意**, 这不是 ReasonReact 的变换; ReasonReact会将最终的列表变成一个数组，但是这个想法仍然适用。
...
所以很自然地，`<MyComponent> myChild </MyComponent>`被去糖(desugar)为`@JSX MyComponent.createElement(~children=list{myChild}, ())`. 也就是说，无论你做什么，传递到 children 位置的参数都会被包裹在一个列表中。如果你不希望这样呢? **如果你想直接传递`myChild`而不需要额外的包装**呢?
...
Reason的 JSX 支持双关语，`<input checked />`只是`<input checked=checked />`的缩写。格式化工具将帮助你尽可能地按照双关语的语法进行格式化，这在有很多 props 需要传递的情况下很方便。
...
这样，每个人都能从 JSX 语法中受益而不需要选择使用它的特定库，如ReasonReact.
...
\* 你可能会想，为什么在 ReactJS 中从来不需要这样的子节点展开呢; ReactJS使用一些特殊的运行时逻辑+特殊的语法转换+变量参数检测和标记来避免大多数的这些情况([见这里](https://github.com/facebook/react/blob/9b36df86c6ccecb73ca44899386e6a72a83ad445/packages/react/src/ReactElement.js#L207)). 这样的动态使用使类型系统复杂化了 _相当多_. 由于我们控制了整个语法和ReasonReact, 我们决定引入子节点展开来区分包装和不包装的情况，而不需要引入编译时和运行时开销。