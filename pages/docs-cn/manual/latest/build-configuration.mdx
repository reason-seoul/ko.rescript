...
你可以把你的目录标记为仅用于开发（例如测试文件）。这些文件不会被构建然后暴露给第三方，或者是同一项目中的其他“开发”目录。
...
注意，只有标有 `"type": "dev"` 的源代码才能够解析到 `bs-dev-dependencies` 中的模块。
...
**从8.4开始**。固定的依赖列表（pinned dependencies）。每当你用 `rescript` 构建一个顶级包（例如你的主程序）时，被固定的依赖关系总是被重新构建。
...
**从9.0开始**：此设置允许依赖外部构建的 stdlib 包（而不是本地构建的 stdlib 运行时）。这对转移只在 JS 或 TS 中使用的包很有用，这样对 ReScript 开发工具链没有任何依赖性。
...
- `"module": "es6-global"` 使用相对路径解析 `node_modules`。这有利于ES6在开发过程中与 Safari 和Firefox等目前支持ES6模块的浏览器结合使用。**没有和开发捆绑**!
