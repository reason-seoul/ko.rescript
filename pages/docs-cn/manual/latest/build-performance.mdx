...
ReScript认为安装时、构建时和运行时的性能，都是很重要的功能；性能属于平时你不在意，但是一旦缺失你却一下就能感受到的事情。
...
有的时候由于一些混乱的基础设置，你的构建过程会很慢。通过 `bstracing`，我们为你的构建提供一个交互可视化工具：
...
ReScript 自身在底层使用了一个名为 [Ninja](https://ninja-build.org) 的构建系统。Ninja和Make类似，但是相比于其他成熟的构建系统来说，它是跨平台的、最小化的、专注于性能的，并且就是为更底层的构建模块而生的。在这方面，Ninja 是 `rescript` 的一个很棒的实现细节。
...
ReScript读取 `bsconfig.json` 并在 `lib/bs` 中生成 Ninja 构建文件。该文件包含底层的的编译器命令、命名空间规则、中间制品的生成和其他内容。最后运行 `ninja` 进行实际构建。
...
如果你不需要监听，你可以运行上述`rescript.exe`。这就避免了Node.js冗长的启动时间，它的启动时间可能在 100ms 左右。我们的编辑器插件会找到并使用这个本地的`rescript.exe`以获得更好的性能。
...
`rescript` 是基于文件的构建系统。 尽管在内存中构建会极大的提升构建速度，但是我们选择不在内存中构建。在内存中构建需要冒着内存泄漏、内存耗尽、构建半途奔溃以及其他风险。我们的监听模式可以整天整月的开着，而不会出现内存泄漏。

<!--
The watcher is also just a thin file watcher that calls `rescript.exe`. We don't like babysitting daemon processes. 
-->

监听器实际上也是一个调用 `rescript.exe` 的小型文件监听器。我们不喜欢保姆守护进程。
...
简而言之，得益于我们的编译器和构建系统架构，我们能够**只构建需要构建的内容**。例如，如果`MyFile.res`没有改变，那么它就不会被重新编译。你可以在JavaScript等语言中粗略地模仿这种增量主义，但不幸的是，其正确程度很低。例如，如果你重命名或移动一个 JS 文件，那么监听器可能会感到困惑，就无法找到“新”文件，或不能正确地清理内容，导致你需要清理你的构建并重新构建一次，这就没有达到增量的目的。
...
ReScript 通过接口文件 (`.resi`) 的概念（等同于 [模块签名](module.md#signatures)），只向外暴露出你需要的东西，这自然会加快增量构建的速度。例如，如果你改变了一个`.res`文件，而其对应的`.resi`文件没有暴露所改变的部分，那么你就减少了你必须重建的依赖文件的数量。
...
然而，热重载在本质上是脆弱的，并且可能会出现偶尔不一致性（错误的状态、错误的解析等），也依赖繁重的开发服务器设置/配置，比起它带来的好处，它带来的混乱更多。我们从谨慎和稳定的角度出发，决定 _暂时_ 不提供内置的热重载。**注意**：你仍然可以使用你的 JS 构建流所提供的热重载设施。
