...
ReScript提供了一个简洁但强大的操作符 `->`，它被叫做“管道”，它允许你“翻转”代码的结构。`a(b)` 可以写成 `b->a`。这是一个简单的语法，并且没有任何运行时开销。
...
这有点难读，因为你需要从最里面的部分开始读，一直到最外面的部分。可以使用管道来精简它：
...
管道可以用来模拟面向对象编程。例如，在 Java 等其他语言中是 `myStudent.getName`，在 ReScript 中可以写为 `myStudent->getName`（相当于 `getName(myStudent)`）。这使我们能够拥有 OOP 的可读性又不必引入一个巨大的类系统。
...
**注意**，只有在管道中才能把变体构造器当成函数使用，在其他任何地方都不行。
...
通过下划线来表达一个占位符，它会告诉 ReScript 你想在之后填入函数在这个位置的参数。下面的两行代码是一个意思：
...
有时你不想把你拥有的值通过管道传到第一个参数，在这些情况下你可以标记一个占位符，来表示你想通过管道传到哪个参数。
...
假设你有一个名为 `namePerson` 的函数，它接受一个 `person` 和一个 `name` 参数。如果你想用管道运算符传递一个 person 可以这样做：
...
如果你有一个 name, 并且想通过管道运算符作用到一个 person 对象上，你可以使用占位符：
...
这让你可以将值通过管道传到任意位置的参数。这种机制对命名参数同样有效：
...
与 `->` 管道不同，`|>` 管道将参数传入最后一个参数（而不是第一个）。`a |> f(b)` 会转变为 `f(b, a)`。
...
关于这两个运算符的联系和区别的更深入讨论，请参考 [Data-first and Data-last comparison by Javier Chávarri](https://www.javierchavarri.com/data-first-and-data-last-a-comparison/)。
