...
模块中的内容（包括类型！）可以像记录一样用 `.` 符号访问。下面的代码演示了模块的命名空间的用途。
...
经常通过模块来引用值/类型是很繁琐的。相反，我们可以 `open` 一个模块并直接使用它的内容，而不必总是在内容前面加上模块的名称。之前这样写：
...
### Use `open!` to ignore shadow warnings | 使用 `open!` 来忽略警告
...
有些情况下，由于重新定义了现有的标识符（绑定、类型），`open` 将会导致警告。使用 `open!` 显式地高速编译器这是所需的行为。
...
**注意：** 与 `open` 一样，如果没有必要，不要过度使用 `open!` 语句。使用（子）模块来防止 shadowing 问题。
...
在模块中使用 `include` 可以静态的”扩展“模块中的内容到新的内容中，因此经常扮演”继承“和“混合”的角色。
...
模块的类型称为“签名”，可以显式地编写。如果模块类似于 `.res` （实现）文件，则模块的签名类似于 `res` （接口）文件。
...
想要创建签名，使用 `module type` 关键字。签名必须以**大写字母** 开头。任何你可以放在 `.res` 文件的东西，你都可以放置在签名定义的 `{}` 块中。
...
像模块本身一样，模块签名也可以通过使用 `include` 的其他模块签名进行扩展：
...
模块可以传递韩素好！它相当于将文件作为第一类子项传递。但是，模块与其他常见概念在语言的“层”上是不同的，因此我们不能将它们传递给常规函数。相反，我们将它们传递给称为“函子”的特殊函数。
...
请注意，具有异常文件名的模块不能从其他 ReScript 模块访问。
...
模块和函子与其他语言（函数、let 绑定、数据结构等）处于不同的“层”。例如，你不能轻易的将它们传递到元祖或记录中。如果需要的话，要明智的使用它们！多次使用，只需要一个记录或一个函数就够了。
