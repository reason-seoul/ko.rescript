...
- Performant and tiny. -->
Rescript-React 附带路由！我们利用语言和库的特性创造的路由功能，有以下特性：
...
可用函数如下：
  ...
  - `RescriptReactRouter.watchUrl(f)`：开始监视 URL 更改。返回一个订阅 token。当 URL 更改后，使用 `RescriptReactRouter.url` 记录作为参数调用回调函数。
  - `RescriptReactRouter.unwatchUrl(watcherID)`：停止监视 URL 更改。
  - `RescriptReactRouter.dangerouslyGetInitialUrl()`：获取 `watchUrl` 之外的 `url` 记录。稍后说明。
  - `RescriptReactRouter.useUrl(~serverUrl)`：返回组件内的 `url`。
...
如果你想要了解更多路由接口实现的底层细节，请参考 [RescriptReactRouter 实现](https://github.com/rescript-lang/rescript-react/blob/master/src/RescriptReactRouter.res)。
...
所以 `www.hello.com/book/10/edit?name=Jane#author` 这样的地址返回：
...
让我们从第一个例子开始，看看 ReScript React 路由是什么样子的：
...
在特定场合中，你可能想在 `watchUrl` 之外保留 `url` 记录。例如，如果你将 `watchUrl` 放在组件的 `didMount` 中，以便 URL 更改时触发组件状态更改，你可能还希望初始状态由 URL 决定。
...
换句话说，如果你需要在应用的逻辑开始时就读取 `url` 记录的内容，为此我们暴露了 `dangerouslyGetInitialUrl()` 接口。
...
注意：我们将其标记为“危险”的原因是提醒你不要在任何组件内读取 `url`，例如 `render`，因为如果所述组件不包含 `watchUrl` 订阅，则 `url` 信息可能已过时。请只将 `dangerouslyGetInitialUrl` 和 `watchUrl` 一起使用。
...
在应用的任何地方，只需要调用例如 `RescriptReactRouter.push("/books/10/edit#validated")` 的命令。这会触发 URL 变更（页面不会刷新），而且 `watchUrl` 的回调会被再次调用。
...
注意：由于浏览器限制，无法检测到通过 JavaScript 更改 URL （又叫 pushState）。解决方案是更改 URL，然后触发 “popState” 事件。这就是 `RescriptReactRouter.push` 所做的事和 `watchUrl` 监听的事件。
因此，如果出于任何原因（例如增量迁移），你需要不通过 `RescriptReactRouter.push` 更新 URL，使用 `window.dispatchEvent(new Event('popState'))`。
...
我们一直努力降低 Rescript-React 的性能开销和学习开销，路由设计也是一样。除浏览器特性检测外，整个路由的实现大约20行代码。回想起来，这样设计似乎是明确的，但是为了达到这个目的，我们必须深入挖掘 ReactJS 内部机制和 future proposals，来确保我们理解了状态更新机制、future context proposal、生命周期排序等概念。并在这个过程中拒绝一些糟糕的 API 设计。很高兴能达成如此明确的解决方案！
...
API 也没有规定路由匹配需要返回组件、状态更新、副作用。它的灵活性足以适配现有的应用。
