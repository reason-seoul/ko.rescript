...
ReScript语言是没有`null`和`undefined`的。这是件 _好事_，因为这抹除了一整类bug。 不会再有 `undefined is not a function` 和 `cannot access someAttribute of undefined` 这种错误出现了!

<!--
However, the **concept** of a potentially nonexistent value is still useful, and safely exists in our language. 
-->

然而，“可能不存在”这一的**概念**仍然是有用的，并且在 ReScript 中是安全的存在的。
...
-->
这意味着：“option类型的值可以是None(表示什么也没有)或者被 Some 包裹的一个真实的值”。 
...
-->
稍后，我们会看到另一些使用这样的值的代码，它会通过[模式匹配](pattern-matching-destructuring.md)让用户强制处理两种分支情况。
...
-->
通过把常规的数字类型转换为`option`类型，并且强制你处理`None`分支，ReScript有效的移除了所有 误处理 或者 忘记处理 一个概念上为`null`的值的可能性! **纯 ReScript 程序不会有null错误**.
...
-->
会编译为`undefined`! 如果你在 JavaScript 中有一个字符串，你知道它可能是 "未定义的", 把它作为 `option<string>`输入就可以了。同样，你可以向 JS 端发送一个`Some(5)`或`None`, 它也能被正确解释 =)
...
-->
`option`到`undefined`的编译并不完美，因为在 ReScript 中，`option`值是可以组合的：
...
- **永远不要标注一个来自于 JS 的值为`option<'a>`类型。每次都把具体的非多态的类型加上。**
...
不幸的是，大部分的时间，你的 JavaScript 值可能 _同时_ 是`null`或`undefined`. 很不幸，在那种情况下，你不能把那个值当作`option<int>`类型，因为当处理`None`的时候，我们的`option`类型只会检查`undefined`而不是`null`.
...
-->
要创建一个 JS 的`null`, 可以使用`Js.Nullable.null`. 要创建一个JS`undefined`, 可以使用`Js.Nullable.undefined` (你自然也可以使用`None`, 但这不是这里的重点; `Js.Nullable.*`里的帮助函数不会对它起作用).
...
-->
如果要从 RsScript 这边创建这样一个可空(nullable)的字符串(先假定是为了把它传递给 JS 那边，以达到互操作的目的)可以这么做：
