...
ReScript提供了一个小但很强大的操作符`->`, 称为 "管道(pipe)", 它允许你从内向外 "翻转"代码。`a(b)`变成`b->a`. 这是一个简单的语法，没有任何运行时成本。
...
管道被用来模拟面向对象的编程。例如，`myStudent.getName`在Java等其他语言中是`myStudent->getName`在ReScript中(相当于`getName(myStudent)`). 这使我们能够拥有OOP的可读性，而没有拖入一个巨大的类系统只是为了在一个数据上调用一个函数。
...
JavaScript的API通常关联在对象上，而且通常是可以链式调用的，像这样：
...
假设我们不需要上面的连锁行为，我们会用上述绑定API页面中的[`@send`](/syntax-lookup#send-decorator)来绑定：
...
这看起来比对应的JS部分还要糟糕得多! 用管道在视觉上把它清理干净：
...
**注意**, 将变体构造函数作为函数使用在除了这里之外的任何地方都不会起作用。

## Pipe Placeholders | 管道占位符
...
通过下划线来表达一个占位符，它告诉ReScript你想在以后填入一个函数的参数。这两者的意义相当：
...
假设你有一个名为`namePerson`的函数，传入一个`person`和一个`name`参数。如果你想用管道运算符传递一个person可以这样做：
...
## Triangle Pipe (Deprecated) | 三角管道运算符(已废弃)
