...
与 React 组件不同，自定义 Hook 不需要具有特定签名。 我们可以决定它接受什么作为参数，以及它应该返回什么（如果有的话）。 换句话说，它就像一个普通的函数。 它的名称应该始终以 use 开头，以便您一眼就可以看出 Hooks 的规则适用于它。
...
**这个代码和之前的例子是等价的吗？**是的，它会以相同的方式工作。如果您仔细观察，您会注意到我们没有对行为进行任何更改。我们所做的只是将两个函数之间的一些公共代码提取到一个单独的函数中。自定义Hooks是自然遵循Hooks设计的约定，而不是React特性。
...
自定义Hooks提供了共享逻辑的灵活性。您可以编写自定义Hook，涵盖广泛的用例，例如表单处理、动画、声明式订阅、计时器，以及可能更多我们没有考虑过的用例。更重要的是，您可以构建与React的内置功能一样易于使用的Hooks。
...
尽量避免过早地添加抽象。当涉及到足够多的状态逻辑处理时，组件会变得非常大是很常见的。这很正常——不要觉得你必须立即将其拆分为Hooks。但我们也鼓励您去发现自定义Hook可以将复杂逻辑隐藏在简单的接口后面或帮助解耦复杂组件的场景。
