...
ReScript 在传统的指令式编程方面有不错的能力。你应当谨慎使用这这些特性，但有时候它们可以使你的代码有更好的性能，也可以让你用一种更加熟悉的方式来写代码。
...
Let 绑定是不可变的，但是你可以用一个`ref`来包裹，这会在标准库中暴露出一个带有单独可变性字段的记录：
...
在使用`ref`之前可以考虑使用[覆盖 let 绑定](let-binding.md#binding-shadowing)来实现轻量的局部“可变性”。
