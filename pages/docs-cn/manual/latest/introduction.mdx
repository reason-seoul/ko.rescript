...
想要一个Javascript一样的编程语言，但是没有先天缺陷，而是有一个更好的类型系统和一整套不会浪费你时间的简洁易用的构建工具链吗？
...
ReScript看起来像JS，运行起来像JS，并且可以编译到简洁、可读且高效的高质量JS代码，还可以直接在浏览器和node中运行。
...
**这意味着你可以随时使用ReScript，访问庞大的JavaScript生态，就好像已经用了很久ReScript一样**
...
我们非常尊重TypeScript，我们认为TypeScript为JavaScript生态起到了积极作用。ReScript与TypeScript有一些相同的目标，但在一些重要的细节上又有足够的不同。
...
- TypeScript的目标是在覆盖整个JavaScript语言特性的基础上提供更多的特性。**ReScript只覆盖了一个精心挑选的JavaScript子集**。 比如说：我们选择 _数据+函数_ 的抽象方式而不是class；使用简洁的模式匹配而不是难以维护的`if`判断和动态分发；使用更好的数据模型而不是字符串区分。
...
\* 当你有编写JavaScript或与之进行互操作的必要时，我们为您敞开了“逃生通道”。
...
ReScript生成的JS代码可读性很好，这对于通过理解代码的编译结果来学习 ReScript 和漏洞审查是十分重要的。
...
我们希望上面的内容能够让你了解到ReScript的思想和与其他语言的差异。请随时尝试我们的[在线编译工具](/try)来感受ReScrit！
