...
**注意：**没有 `else` 分支的 `if-else` 表达式会隐式补充上 `()`（`unit` 类型）来作为 `else` 分支的值。所以：
...
它会抛出一个类型错误，隐含的 `else` 分支的类型是 `unit`，而 `if` 分支的类型是 `int`。从直觉上来看这是有道理的：如果 `showMenu` 是 `false`，那 `result` 的值又应该是什么呢？
...
**`if-else` 和三目运算符在 ReScript 中的使用比其他语言少得多**；[模式匹配](pattern-matching-destructuring.md)消灭了一大堆以前需要条件表达式的代码。
...
在 ReScript 中没有用于跳出循环的 `break` 关键字（也没有从函数中提前返回的 `return`）。不过我们可以通过使用[可变绑定](mutation.md)轻松地脱离 while 循环。
